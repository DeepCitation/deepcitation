...
- **Anchor is pinned to `sourceContextDeepItem`'s bbox center — not derived from `evidence.textItems`** — `CitationAnnotationOverlay` draws the page-view spotlight directly from `sourceContextDeepItem`. The resolver returns the center of that same bbox so the ghost's frame-0 position is pixel-identical to the spotlight — zero drift by construction, regardless of what `evidence.textItems` happens to contain (wider parent items, char-level fragments, stray common-word matches, missing items after crop). Item-based matching is retained only as a fallback for legacy payloads without `sourceContextDeepItem` (URL citations). The invariant is enforced by the "anchor ↔ spotlight invariant" block in `src/__tests__/resolvers.test.ts`, which computes the expected value the same way the overlay does (via `sourceContextDeepItem` center) — not by hand-picking expected ratios from the same inputs the resolver sees. The hand-picked approach is how `collapse4/5/6.png` kept slipping through six TDD iterations: the resolver unit tests passed while the animation still visibly offset, because the "expected" ratios encoded the same bug the resolver had. Tests that describe anchor behavior on wrapped / row / fragment-match cases must compare against `spotlightCenterRatio(v)` (derived from `sourceContextDeepItem`), not against an invented literal.
...
- Do not revert `resolveEvidenceSourceAnchorRatio` to deriving the anchor from `evidence.textItems` when `sourceContextDeepItem` is available — see "Anchor is pinned to `sourceContextDeepItem`'s bbox center" above. The item-matching path is the fallback; the primary path must read the spotlight's bbox directly so the two pipelines cannot disagree. Any regression here re-opens the `collapse6.png` class of bugs (parent-item drift, char-fragment drift, noise-pulled centroid) which are invisible to single-input resolver tests but visibly offset the ghost on-screen.
