...
**Projected width seed must also not upscale**: `projectedSummaryKeyholeWidth` in `usePopoverPosition.ts` and the matching `keyholeDisplayedWidth` seed in `DefaultPopoverContent.tsx` both delegate to `projectKeyholeDisplayedWidth()` in `react/constants.ts`, which clamps zoom to `Math.min(1, STRIP_HEIGHT / height)`. The `min(1, …)` is load-bearing — the previous formula `width × (STRIP_HEIGHT / height)` projected phantom upscaled widths (e.g. 1200×80 → 1800) for short images, causing the popover to render too wide and then pop narrower once the real keyhole measured in. `AnchorTextFocusedImage.stripHeightStyle` (above) is the third load-bearing site: it encodes the same invariant via CSS `min()` on the container height. All three sites must move together. Covered by `src/__tests__/projectKeyholeDisplayedWidth.test.ts`.
