...
import { ValidationError, getAllCitationsFromLlmOutput, getCitationStatus, getVerificationTextIndicator, sanitizeForLog, } from "deepcitation";
import { wrapCitationPrompt } from "deepcitation/prompts";
import { EventEncoder } from "@ag-ui/encoder";
import OpenAI from "openai";
import { runStarted, textMessageStart, textMessageContent, textMessageEnd, stateDelta, stateSnapshot, runFinished, runError, } from "@/lib/agui-events";
...
async function resolveAttachment(dcClient, source) {
    const savedId = process.env[source.attachmentEnvVar];
    if (savedId) {
        try {
            const attachment = await dcClient.getAttachment(savedId);
            const attachmentPages = attachment.deepTextPages ??
                attachment.pageTexts ??
                [];
            if (attachmentPages.length) {
                return { attachmentId: savedId, deepTextPages: attachmentPages };
            }
            console.warn(`[DeepCitation] ${source.attachmentEnvVar}=${savedId} did not return deepTextPages — re-uploading.`);
        }
        catch (err) {
            console.warn(`[DeepCitation] Cached ${source.attachmentEnvVar}=${savedId} failed (${err instanceof Error ? err.message : err}) — re-uploading.`);
        }
    }
    ...
    console.log(`[DeepCitation] Uploaded "${source.title}". Add to env to skip re-upload on cold starts:\n  ${source.attachmentEnvVar}=${attachmentId}`);
    return { attachmentId, deepTextPages };
}
function cacheAttachment(dcClient, source) {
...
function getAttachmentPromise(dcClient, source) {
...
export async function POST(req) {
    ...
    const clientFileDataParts = state?.fileDataParts ?? [];
    const clientDeepTextPagesByAttachmentId = state?.deepTextPagesByAttachmentId ?? {};
    if (!openai) {
        ...
        async start(controller) {
            ...
            try {
                ...
                if (fileDataParts.length === 0 && dc) {
                    const corpusResults = await Promise.all(CORPUS_SOURCES.map(source => getAttachmentPromise(dc, source)));
                ...
                openaiMessages.push({ role: "user", content: enhancedUserPrompt });
                ...
                if (hasDocuments && dc && fullResponse) {
                    ...
                    if (citationCount > 0) {
                        ...
                        for (const [key, citation] of citationEntries) {
                            console.log(`[agui-chat] Citation ${key}: anchor="${citation.sourceMatch ?? ""}" full="${citation.sourceContext ?? ""}" ` +
                                `pageId="${citation.startPageId ?? ""}" lineIds="${citation.lineIds?.join(",") ?? ""}"`);
                        }
                        let result;
                        try {
                            result = await dc.verify({ llmOutput: fullResponse, outputImageFormat: "avif" }, citations);
                        }
                        catch (err) {
                        ...
                        for (const [key, verification] of Object.entries(verifications)) {
                            ...
                            const status = getCitationStatus(verification);
                            console.log(`[agui-chat] Verification ${key}: ${indicator} status="${verification.status ?? "unknown"}" ` +
