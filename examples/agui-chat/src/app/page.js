...
export default function Home() {
    ...
    const { messages, isLoading, isVerifying, error, messageVerifications, sendMessage, retry, } = useAgentChat({
        agentUrl: "/api/agent",
        fileDataParts,
    ...
    const handleSubmit = (e) => {
        ...
        if (!trimmed || isLoading || isVerifying)
            return;
        sendMessage(trimmed);
        setInput("");
    };
    ...
    const latestVerification = messages.length > 0
    ...
    return (<div className="flex h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {messages.length === 0 ? (<div className="flex flex-col items-center justify-center h-full text-center">
              ...
              {error && (<div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">
                  ...
                  {messages.length > 0 && (<button onClick={() => {
                        const lastAssistant = messages.findLast(m => m.role === "assistant");
                        if (lastAssistant)
                            retry(lastAssistant.id);
        ...
        <div className="border-t bg-white px-6 py-4">
          {messages.length === 0 && (<div className="flex flex-wrap gap-2 mb-3">
              {SAMPLE_QUESTIONS.map(sample => (<button key={sample} type="button" onClick={() => handleSampleQuestion(sample)} disabled={isLoading || isVerifying} className="px-3 py-1.5 text-sm text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-left">
                  {sample}
                </button>))}
            </div>)}
          ...
          <form onSubmit={handleSubmit} className="flex gap-3">
            ...
            <input type="text" value={input} onChange={e => setInput(e.target.value)} placeholder="Ask a question about the corpus documents..." className="flex-1 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" disabled={isLoading || isVerifying}/>
            <button type="submit" disabled={isLoading || isVerifying || !input.trim()} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
              Send
            </button>
          </form>
        ...
        <div className="p-4 space-y-5">
          ...
          <section>
            ...
            <ul className="space-y-1.5">
              {CORPUS_SOURCES.map(doc => (<li key={doc.filename}>
                  <a className="text-xs text-blue-600 hover:text-blue-800 hover:underline flex items-center gap-1" href={`/api/corpus/${doc.filename}`} target="_blank" rel="noreferrer">
                    <span>{doc.title}</span>
