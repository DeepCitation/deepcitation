"use client";
import { CITATION_DATA_END_DELIMITER, parseCitationResponse, } from "deepcitation";
...
export function ChatMessage({ message, citations, verifications, drawerItems }) {
    ...
    return (<div className={`flex gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
      {!isUser && (<div className="size-8 rounded-full bg-blue-600 flex items-center justify-center text-white text-sm font-medium shrink-0">
          AI
        </div>)}
      ...
      <div className={`max-w-[80%] rounded-lg px-4 py-3 ${isUser ? "bg-blue-600 text-white" : "bg-white border shadow-sm"}`}>
        {isUser ? (<p className="whitespace-pre-wrap">{message.content}</p>) : (<>
            ...
            {citationGroups.length > 0 && (<div className="mt-3 pt-2 border-t border-zinc-100">
                <CitationDrawerTrigger citationGroups={citationGroups} onClick={() => setDrawerOpen(true)} isOpen={drawerOpen}/>
              </div>)}
          </>)}
      </div>

      {isUser && (<div className="size-8 rounded-full bg-zinc-300 flex items-center justify-center text-zinc-700 text-sm font-medium shrink-0">
          U
        </div>)}
...
function MarkdownWithCitations({ visibleText, markerMap, parsedCitations, citations, verifications, }) {
    ...
    const resolveCitation = useCallback((n) => {
    ...
    const components = useMemo(() => ({
        "citation-marker": ({ n }) => resolveCitation(n),
        code: ({ inline, className, children }) => {
            ...
            if (!inline && (language === "text" || language === "txt")) {
                ...
                return (<pre className="bg-zinc-50 border border-zinc-200 rounded p-3 text-sm overflow-auto">
            <code>{nodes}</code>
          </pre>);
            }
