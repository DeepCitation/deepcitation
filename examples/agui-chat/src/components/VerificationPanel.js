"use client";
import { isValidProofImageSrc } from "deepcitation/react";
...
export function VerificationPanel({ verification }) {
    ...
    return (<div className="w-80 border-l bg-white flex flex-col">
      {/* Header */}
      ...
      <div className="px-4 py-3 border-b bg-gray-50">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className={`text-sm font-medium ${verificationRate >= 80 ? "text-green-600" : verificationRate >= 50 ? "text-yellow-600" : "text-red-600"}`}>
        ...
        <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
          <div className={`h-full transition-all ${verificationRate >= 80 ? "bg-green-500" : verificationRate >= 50 ? "bg-yellow-500" : "bg-red-500"}`} style={{ width: `${verificationRate}%` }}/>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {Object.entries(verifications).map(([key, v]) => {
            ...
            return (<div key={key} className="border-b last:border-b-0">
              <button onClick={() => setExpandedCitation(isExpanded ? null : key)} className="w-full px-4 py-3 flex items-center gap-3 hover:bg-gray-50 text-left">
                {/* Status icon */}
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-sm ${isVerified
              ...
              {isExpanded && (<div className="px-4 pb-3 space-y-2">
                  {v.verifiedSourceMatch && (<div className="bg-gray-50 rounded p-2">
                  ...
                  {evidenceSrc && isValidProofImageSrc(evidenceSrc) ? (<div className="bg-gray-50 rounded p-2">
                  ...
                  {hasPageMismatch && (<div className="text-xs text-yellow-600">
