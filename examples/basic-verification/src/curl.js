...
import { extractVisibleText, getAllCitationsFromLlmOutput, getCitationStatus, getVerificationTextIndicator, replaceCitationMarkers, } from "deepcitation";
import { wrapCitationPrompt } from "deepcitation/prompts";
import { readFileSync } from "fs";
import OpenAI from "openai";
import { dirname } from "path";
import { fileURLToPath } from "url";
import { SOURCES, promptSourceSelection } from "./shared.js";
 ...
 */
async function prepareFileAttachment(file, filename) {
 ...
 */
async function prepareUrlAttachment(url) {
    const response = await fetch(`${DEEPCITATION_BASE_URL}/prepareAttachments`, {
        method: "POST",
        headers: {
            Authorization: `Bearer ${DEEPCITATION_API_KEY}`,
 ...
 */
async function verifyCitations(attachmentId, citations) {
    ...
    if (verifications.length === 0) {
        ...
        for (const [key, verification] of verifications) {
            ...
            if (originalCitation?.sourceContext) {
                console.log(`  📝 Claimed: "${originalCitation.sourceContext.slice(0, 100)}${originalCitation.sourceContext.length > 100 ? "..." : ""}"`);
            }
...
async function main() {
    ...
    if (process.env.SOURCE === "all") {
        ...
        const s = SOURCES[idx];
        if (!s)
            throw new Error(`Invalid SOURCE=${idx}. Valid range: 0-${SOURCES.length - 1}`);
        sources = [s];
    }
    else {
