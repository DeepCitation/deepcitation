...
import { SOURCES, DEFAULT_OUT_DIR, toSafeName, stepUpload, stepWrapPrompts, stepCallLlm, stepParseCitations, stepVerify, stepGenerateHtml, } from "./shared.js";
// ─── CLI arg parsing ───────────────────────────────────────────────────────
function parseArgs() {
    const flags = {};
    const positional = [];
...
// ─── Persistence helpers ───────────────────────────────────────────────────
const STEP_LABELS = ["upload", "prompts", "llm", "parsed", "verified", "html"];
function stepPath(cacheDir, safeName, step) {
    return resolve(cacheDir, `${safeName}-step${step}-${STEP_LABELS[step - 1]}.json`);
}
function saveStep(cacheDir, safeName, step, data) {
...
function loadStep(cacheDir, safeName, step) {
    const path = stepPath(cacheDir, safeName, step);
    if (!existsSync(path)) {
...
function resolveSource(arg) {
    ...
    return { type, path: filePath, filename, label: filename };
}
...
if (from > 1) {
    ...
    if (from <= 6)
        s1 = loadStep(cacheDir, safeName, 1); // needed by steps 2,3,5,6
    if (from > 2 && to >= 3)
        s2 = loadStep(cacheDir, safeName, 2); // needed by step 3
    if (from > 3)
        s3 = loadStep(cacheDir, safeName, 3); // needed by step 4
    if (from > 4)
        s4 = loadStep(cacheDir, safeName, 4); // needed by steps 5,6
    if (from > 5)
        s5 = loadStep(cacheDir, safeName, 5); // needed by step 6
    console.log();
}
...
if (from <= 6 && to >= 6) {
    ...
    try {
        ...
        try {
            execFileSync("xdg-open", [s6.htmlPath], { stdio: "ignore", timeout: 5000 });
        }
        catch {
            try {
                execFileSync("open", [s6.htmlPath], { stdio: "ignore", timeout: 5000 });
            }
            catch { /* manual */ }
        }
    }
...
const VERIFIED_STATUSES = new Set(["found", "found_context_missed_source_match"]);
timing.total_ms = Date.now() - runStart;
if (s5?.verifications) {
    const verifs = Object.values(s5.verifications);
