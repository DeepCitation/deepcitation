...
function findNewestSnapshot(): string {
  if (!existsSync(DEFAULT_OUT)) fail(`Output directory does not exist: ${DEFAULT_OUT}`);
...
function parseArgs(): { snapshotPath: string; mdOverride?: string } {
  let mdOverride: string | undefined;
  let positional: string | undefined;
  for (const arg of process.argv.slice(2)) {
    if (arg.startsWith("--md=")) mdOverride = arg.slice(5);
...
if (!snapshot.llmResponse) fail(`Snapshot has no llmResponse field: ${snapshotPath}`);
...
if (!existsSync(mdPath)) writeFileSync(mdPath, visibleText);
