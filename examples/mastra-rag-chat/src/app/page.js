    ...
    const sendQuestion = async (nextQuestion) => {
        ...
        const response = await fetch("/api/chat", {
            ...
            if (result.status === 504) {
                throw new Error("The server timed out. This usually happens on the first request after a cold start — please try again in a moment.");
            }
            let payload;
            try {
        ...
        <section className="chat-panel panel">
          ...
          <div className="messages">
            {messages.length === 0 ? (<div className="empty-state">
              ...
              </div>) : (<div className="message-stack">
          ...
          <div className="composer">
            <form className="composer-form" onSubmit={async (event) => {
            event.preventDefault();
            await sendQuestion(question);
        }}>
              {messages.length === 0 && (<div className="sample-list">
                  {SAMPLE_QUESTIONS.map(sample => (<button key={sample} type="button" className="sample-button" onClick={() => sendQuestion(sample)} disabled={isLoading}>
                      {sample}
                    </button>))}
                </div>)}
              <div className="composer-row">
                <textarea className="question-input" value={question} onChange={event => setQuestion(event.target.value)} placeholder="Ask a question about the bundled PDFs." disabled={isLoading}/>
