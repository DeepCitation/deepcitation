...
On page load, the client calls `GET /api/corpus/init` which resolves the sample PDF attachment server-side (with env-var caching for fast cold starts). The `fileDataPart` and `deepTextPages` are sent to the client so it can pass them through `useChat`'s body — same flow as a user-uploaded file.
