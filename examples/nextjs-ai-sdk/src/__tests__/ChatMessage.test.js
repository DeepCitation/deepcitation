...
vi.mock("deepcitation/react", () => ({
    CitationComponent: ({ citation }) => (<span data-testid="citation">[{citation?.sourceContext || "citation"}]</span>),
}));
describe("ChatMessage", () => {
    ...
    it("renders user message without crashing", () => {
        render(<ChatMessage message={{ ...baseMessage, role: "user" }} citations={undefined} verifications={undefined} summary={undefined}/>);
    ...
    it("renders assistant message with empty citations/verifications", () => {
        render(<ChatMessage message={baseMessage} citations={{}} verifications={{}} summary={{ total: 0, verified: 0, missed: 0, pending: 0 }}/>);
        expect(document.body).toBeInTheDocument();
    });
    it("renders message with content from parts array", () => {
        render(<ChatMessage message={{
            ...
            }} citations={undefined} verifications={undefined} summary={undefined}/>);
        expect(document.body).toBeInTheDocument();
    });
    it("renders citations when provided", () => {
        ...
        render(<ChatMessage message={{
            ...
            }} citations={citations} verifications={verifications} summary={{ total: 1, verified: 1, missed: 0, pending: 0 }}/>);
        expect(document.body).toBeInTheDocument();
    });
});
