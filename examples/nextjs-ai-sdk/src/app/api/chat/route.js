...
export async function POST(req) {
    ...
    if (!allowed) {
        ...
        const midnightMs = new Date(new Date().toISOString().slice(0, 10) + "T00:00:00Z").getTime() + 86_400_000;
    ...
    const fileDataParts = clientFileDataParts;
    ...
    const enhancedUIMessages = uiMessages.map((m, i) => {
        if (i === uiMessages.length - 1 && m.role === "user" && hasDocuments) {
            return {
                ...m,
                parts: [{ type: "text", text: enhancedUserPrompt }],
            };
        }
        return m;
    });
