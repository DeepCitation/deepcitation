...
import { extractVisibleText, getAllCitationsFromLlmOutput, getCitationStatus, groupCitationsByAttachmentId, replaceCitationMarkers, } from "deepcitation";
...
async function getStore() {
    storePromise ??= (async () => {
        ...
        console.log(`   indexed=${updateResult.indexed} updated=${updateResult.updated} unchanged=${updateResult.unchanged}`);
...
// ─── md → pdf mapping ───────────────────────────────────────────────────────
function mdFileToPdfPath(mdFile) {
...
// ─── DeepCitation upload ────────────────────────────────────────────────────
async function uploadAttachments(dc, hits) {
    const uploads = hits.map(hit => {
        const pdfPath = mdFileToPdfPath(hit.file);
        if (!existsSync(pdfPath)) {
            throw new Error(`Missing parallel PDF for ${hit.file}. Expected ${pdfPath}. Run "bun run build:corpus".`);
        }
...
export async function runWorkflow(providerName, streamLlm) {
    ...
    // ── Step 3: Wrap prompts ──────────────────────────────────────────────
    const systemPrompt = "You are a precise research assistant. Answer only from the retrieved documents. Cite every factual claim.";
    const userPrompt = [
        `Question: ${question}`,
        "",
    ...
    const { enhancedSystemPrompt, enhancedUserPrompt } = wrapCitationPrompt({
        systemPrompt,
        userPrompt,
        deepTextPagesByAttachmentId: Object.fromEntries(prepared.map(item => [item.attachmentId, item.deepTextPages])),
    });
    // ── Step 4: Call LLM ──────────────────────────────────────────────────
    ...
    for (const [attachmentId, attachmentCitations] of grouped) {
        ...
        if (result.attachments)
            Object.assign(mergedAttachments, result.attachments);
    }
