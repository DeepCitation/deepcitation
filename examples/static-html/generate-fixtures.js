...
import { DeepCitation, extractVisibleText, getAllCitationsFromLlmOutput, wrapCitationPrompt, } from "deepcitation";
...
async function main() {
    ...
    const fixtures = {};
    const keyMap = {}; // original hash → c1, c2, ...
    let idx = 1;
    for (const [hashKey, v] of Object.entries(result.verifications)) {
        ...
        if (filteredPageImages && matchPage != null) {
            filteredPageImages = filteredPageImages.filter((p) => Math.abs(p.pageNumber - matchPage) <= 1);
        }
        fixtures[simpleKey] = {
    ...
    for (const [hash, simple] of Object.entries(keyMap)) {
        const status = fixtures[simple].status;
