...
async function main() {
    ...
    const config = JSON.parse(fs.readFileSync(configAbsolute, "utf-8"));
    ...
    const prepared = prepareResults.filter((r) => r !== null);
    if (prepared.length === 0) {
        console.error("No sources prepared successfully");
        process.exit(1);
    }
    // Build lookups
    const urlToAttachmentId = new Map();
    const nameToAttachmentId = new Map();
    const attachmentIdToName = new Map();
    for (const s of prepared) {
        if (s.url)
            urlToAttachmentId.set(s.url, s.attachmentId);
    ...
    for (let i = 0; i < config.citations.length; i++) {
        ...
        if (citation.type === "url" && citation.url) {
            attachmentId = urlToAttachmentId.get(citation.url);
        }
    ...
    const verificationResults = new Array(config.citations.length).fill(null);
        ...
        for (let j = 0; j < group.citations.length; j++) {
            ...
            if (!v) {
                ...
                verificationResults[originalIndex] = {
                    citation,
                    verification: { status: "not_found", verifiedAt: new Date().toISOString() },
    ...
    const output = {
        ...
        results: verificationResults.filter((r) => r !== null),
    };
    if (outputPath) {
