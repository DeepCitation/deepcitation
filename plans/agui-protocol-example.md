...
3. Emit `RUN_STARTED`
4. Build `deepTextPages` from uploaded documents — this is the extracted text content from `prepareAttachments()` responses, stored in client state as `deepTextPages`. It is a DeepCitation-specific field, not an AG-UI convention.
5. Call `wrapCitationPrompt({ deepTextPages, systemPrompt, userPrompt: userMessage })`
