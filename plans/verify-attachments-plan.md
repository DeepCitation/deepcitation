...
## Appendix: URL-Based Batch Verification Test Plan

Batch mode is especially valuable for URL sources, where a single analysis may reference multiple web pages. This appendix defines a concrete test using legal URLs.

### Test: Multi-URL Legal Analysis (Batch Mode)

**Setup**: User provides 3 legal URLs and asks for a comparative analysis.

```
/verify https://www.law.cornell.edu/supct/html/12-307.ZO.html https://www.law.cornell.edu/uscode/text/42/chapter-21/subchapter-I-A https://www.ecfr.gov/current/title-29/subtitle-B/chapter-XIV/part-1630
```

**User prompt**: "Compare how the Voting Rights Act, its judicial interpretation in Shelby County, and the ADA employment regulations handle enforcement mechanisms"

**Expected behavior with batch mode**:
1. `prepare` each URL → 3 separate `attachmentId` values
2. Build citations spanning all 3 sources (Shelby County opinion, VRA statute, ADA regs)
3. Send ONE batch verify request with `"mode": "batch"` containing all citations
4. Each citation carries its own `attachmentId`
5. Single merged response

**Without batch mode** (current): 3 separate verify calls, manual merge.

**Validation**:
- Confirm citations reference the correct `attachmentId` for each source
- Confirm cross-source claims (e.g., "the Court struck down Section 4(b)" referencing both the opinion and the statute) cite the appropriate source
- Serve the output HTML and capture Playwright screenshots of popovers from each source

**Playwright screenshots** (saved to `scratch/legal-qa/batch-test/`):
- `popover-shelby-holding.png` — citation from the court opinion
- `popover-vra-section2.png` — citation from the statute text
- `popover-ada-definition.png` — citation from the regulation
- Each should show the source URL/domain in the popover metadata
