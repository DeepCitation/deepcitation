...
describe("PagePicker", () => {
    it("renders descriptive pills for active and neighbor pages, dots for distant pages (expanded state)", () => {
        ...
        const { getAllByRole } = render(<PagePicker pages={[1, 2, 3, 4, 5, 6]} activePage={3} onPageClick={onPageClick} isExpanded={true}/>);
        expect(getAllByRole("group")[0]).toHaveAttribute("aria-label", "Page navigation");
        // Active pill (p.3): button with X close treatment
        const closeButtons = getAllByRole("button").filter(btn => btn.getAttribute("aria-label")?.includes("Close") && btn.getAttribute("aria-label")?.includes("3"));
        ...
        const neighborButtons = getAllByRole("button").filter(btn => {
            ...
            return ((label.includes("Expand") || label.includes("page 2") || label.includes("page 4")) && !label.includes("Close"));
        });
        expect(neighborButtons.length).toBeGreaterThanOrEqual(2);
        // Dots (p.1, p.5, p.6): small dot buttons with aria-label "Go to page N"
        const dotButtons = getAllByRole("button").filter(btn => btn.hasAttribute("data-dc-page-dot"));
        expect(dotButtons.length).toBeGreaterThanOrEqual(3);
    });
    it("active pill shows chevron (expand) label when not in expanded state", () => {
        const onPageClick = mock(() => { });
        const { getAllByRole } = render(<PagePicker pages={[1, 2, 3, 4, 5, 6]} activePage={3} onPageClick={onPageClick} isExpanded={false}/>);
    ...
    it("fires onPageClick with the correct page when a neighbor pill is clicked", () => {
        ...
        if (neighbor4)
            fireEvent.click(neighbor4);
        const calls = onPageClick.mock.calls;
    ...
    it("two-page input renders two descriptive pills with no dots", () => {
        const onPageClick = mock(() => { });
        const { getAllByRole } = render(<PagePicker pages={[2, 3]} activePage={2} onPageClick={onPageClick} isExpanded={true}/>);
        const allButtons = getAllByRole("button");
        // Both pages are active or neighbor — no dots
        const dotButtons = allButtons.filter(btn => btn.hasAttribute("data-dc-page-dot"));
