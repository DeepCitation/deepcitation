...
describe("SupportingFactsPills", () => {
    ...
    it("renders a region with the 'Supporting facts' aria-label", () => {
        const { getByRole } = render(<SupportingFactsPills parentCitation={parentCitation} parentKey={parentKey} supportingFacts={facts}/>);
        const region = getByRole("region", { name: "Supporting facts" });
        expect(region).toBeInTheDocument();
    });
    it("renders one chip button per supporting fact", () => {
        const { getAllByRole } = render(<SupportingFactsPills parentCitation={parentCitation} parentKey={parentKey} supportingFacts={facts}/>);
    ...
    it("renders pills without throwing when supportingFactVerifications is provided", () => {
        const v0 = { status: "not_found" };
        const v1 = { status: "found" };
        const { getAllByRole } = render(<SupportingFactsPills parentCitation={parentCitation} parentKey={parentKey} supportingFacts={facts} supportingFactVerifications={[v0, v1]}/>);
    ...
    it("passes parentInstanceId down so child popovers don't close the parent", () => {
        ...
        const { getAllByRole } = render(<SupportingFactsPills parentCitation={parentCitation} parentKey={parentKey} supportingFacts={facts} parentInstanceId={parentId}/>);
        expect(getAllByRole("button")).toHaveLength(facts.length);
    });
});
