...
describe("VerificationTabs", () => {
    ...
    describe("exact match state", () => {
        it("displays exact match badge when texts are identical", () => {
            const { container } = render(<VerificationTabs expected="Hello world" actual="Hello world"/>);
        ...
        it("displays the matched text content", () => {
            const { container } = render(<VerificationTabs expected="Matched text" actual="Matched text"/>);
    ...
    describe("tab navigation", () => {
        it("renders all three tabs when texts differ", () => {
            ...
            expect(within(tabsNav).getByText("Expected")).toBeInTheDocument();
            expect(within(tabsNav).getByText("Diff")).toBeInTheDocument();
            expect(within(tabsNav).getByText("Found")).toBeInTheDocument();
        });
        ...
        it("stays on Diff tab with split view for high variance", async () => {
            ...
            await waitFor(() => {
                ...
                expect(splitViewButton).toHaveClass("bg-dc-muted");
            });
        });
        ...
        it("stops event propagation on tab click", () => {
            const onClick = mock(() => { });
            const { container } = render(<div role="button" tabIndex={0} onClick={onClick} onKeyDown={(e) => {
    ...
    describe("Diff tab content", () => {
        ...
        it("displays Exact Match indicator when no diff but tab is viewed", () => {
            ...
            const { rerender, container } = render(<VerificationTabs expected="Different" actual="Text"/>);
    ...
    describe("Found tab content", () => {
        it("displays actual text when Found tab is active", () => {
            const { container } = render(<VerificationTabs expected="Expected" actual="Found content"/>);
        ...
        it("shows empty text message when actual is empty", () => {
            const { container } = render(<VerificationTabs expected="Expected" actual="" emptyText="Nothing found"/>);
            const foundTab = within(container).getByText("Found");
            fireEvent.click(foundTab);
            expect(within(container).getByText("Nothing found")).toBeInTheDocument();
        });
        it("uses default empty text when not provided", () => {
            const { container } = render(<VerificationTabs expected="Expected" actual=""/>);
            const foundTab = within(container).getByText("Found");
            fireEvent.click(foundTab);
        ...
        it("renders copy button for found text when provided", () => {
            const renderCopyButton = mock((_text, position) => <button type="button">Copy {position}</button>);
            const { container } = render(<VerificationTabs expected="Expected" actual="Actual" renderCopyButton={renderCopyButton}/>);
            const foundTab = within(container).getByText("Found");
            fireEvent.click(foundTab);
    ...
    describe("label prop", () => {
        it("displays label when provided", () => {
            const { container } = render(<VerificationTabs expected="Expected" actual="Actual" label="Verification Results"/>);
            expect(within(container).getByText("Verification Results")).toBeInTheDocument();
        });
    ...
    describe("renderCopyButton prop", () => {
        ...
        it("renders custom copy button for expected tab", () => {
            const renderCopyButton = mock(text => (<button type="button" data-testid="custom-copy">
          {text}
        </button>));
            const { container } = render(<VerificationTabs expected="Hello world" actual="Hello universe" renderCopyButton={renderCopyButton}/>);
            const tabsNav = container.querySelector('[data-testid="tabs-nav"]');
    ...
    describe("edge cases", () => {
        ...
        it("handles empty actual text", () => {
            const { container } = render(<VerificationTabs expected="Some text" actual=""/>);
            const foundTab = within(container).getByText("Found");
            fireEvent.click(foundTab);
        ...
        it("handles very long texts", () => {
            ...
            const { container } = render(<VerificationTabs expected={longText} actual={longText}/>);
            expect(within(container).getByText("Exact match")).toBeInTheDocument();
        });
