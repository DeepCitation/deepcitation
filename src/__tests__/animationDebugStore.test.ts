...
import {
  ...
  setLastGhostRects,
  subscribeDebug,
} from "../react/debug/animationDebugStore.js";
...
describe("animationDebugStore", () => {
  ...
  describe("console API", () => {
    ...
    it("setSpeed clamps to [0.05, 10]", () => {
      const api = (window as unknown as { __dcAnimationDebug: { setSpeed: (x: number) => void } }).__dcAnimationDebug;
      api.setSpeed(0.01);
    ...
    it("scrub sets frozen state", () => {
      const api = (
        window as unknown as {
          __dcAnimationDebug: {
            enable: () => void;
            scrub: (p: number, kind?: string) => void;
          };
        }
      ).__dcAnimationDebug;
      api.enable();
    ...
    it("step mutates activeAnim.currentTime", () => {
      const api = (window as unknown as { __dcAnimationDebug: { step: (ms: number) => void } }).__dcAnimationDebug;
      let currentTime: number | CSSNumericValue | null = 100;
      const fakeAnim = {
        get currentTime() {
          return currentTime;
        },
        set currentTime(v: number | CSSNumericValue | null) {
          currentTime = v;
        },
        pause: () => {},
        play: () => {},
