...
import { generateNonce, maskKey, openBrowser, resolveAuth, sourceLabel, startCallbackServer, } from "../auth.js";
...
async function cleanup(port, nonce) {
    try {
...
describe("sourceLabel", () => {
    ...
    it("returns path for credentials source", () => {
        expect(sourceLabel({ kind: "credentials", path: "/home/user/.deepcitation/credentials.json" })).toBe("/home/user/.deepcitation/credentials.json");
    });
});
// ── openBrowser ────────────────────────────────────────────────────
describe("openBrowser", () => {
    ...
    it("uses explorer.exe on Windows (no shell interpreter)", () => {
        ...
        expect(execFileMock).toHaveBeenCalledWith("explorer.exe", ["https://deepcitation.com/auth?token=abc&nonce=xyz"], expect.any(Function));
    ...
    it("uses open on macOS", () => {
        ...
        expect(childProcess.execFile).toHaveBeenCalledWith("open", ["https://deepcitation.com"], expect.any(Function));
    });
    it("uses wslview on Linux", () => {
        setPlatform("linux");
        openBrowser("https://deepcitation.com");
        expect(childProcess.execFile).toHaveBeenCalledWith("wslview", ["https://deepcitation.com"], expect.any(Function));
    });
