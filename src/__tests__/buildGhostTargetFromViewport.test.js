...
function makeSnapshot(opts) {
    const { viewportW, viewportH, imageOffsetLeft = 0, imageOffsetTop = 0, imageWidth = viewportW, imageHeight = viewportH, } = opts;
    return {
...
function makeRoot(containerRect, imgRect) {
...
describe("buildGhostTargetFromViewport — ghost dimensions invariant", () => {
    ...
    test("ghostRect visible viewport center lands on the visible page center", () => {
        ...
        expect(result).not.toBeNull();
        if (result == null)
            return; // narrow for TypeScript — expect above handles failure
