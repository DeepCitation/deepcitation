...
function getIconSpan(container) {
    const pill = container.querySelector("[data-dc-indicator]");
    // Inner span is the direct child span that holds the SVG
    const inner = pill?.querySelector(":scope > span");
    return inner;
}
describe("Caret Indicator Variant", () => {
    ...
    it("renders pill wrapper with rounded-full and SVG inside inner span", () => {
        ...
        const pill = container.querySelector("[data-dc-indicator='caret']");
    ...
    it("uses caret-error data attribute and red classes when miss", () => {
        const { container } = render(<CitationStatusIndicator {...baseProps} isVerified={false} isMiss={true} status={"not_found"}/>);
        const pill = container.querySelector("[data-dc-indicator='caret-error']");
        expect(pill).toBeInTheDocument();
    ...
    it("renders gray caret for partial status", () => {
        const { container } = render(<CitationStatusIndicator {...baseProps} isVerified={false} isPartialMatch={true} status={"partial"}/>);
        const pill = container.querySelector("[data-dc-indicator='caret']");
        expect(pill).toBeInTheDocument();
        expect(pill.classList.contains("text-dc-subtle-foreground")).toBe(true);
    });
    ...
    it("wraps spinner in pill with rounded-full", () => {
        const { container } = render(<CitationStatusIndicator {...baseProps} shouldShowSpinner={true}/>);
        const pill = container.querySelector("[data-dc-indicator='pending']");
        expect(pill).toBeInTheDocument();
        expect(pill.classList.contains("rounded-full")).toBe(true);
    });
    // ==========================================================================
    ...
    describe("CARET_INDICATOR_SIZE_STYLE", () => {
        it("is between dot and icon size", () => {
            ...
            expect(Number.parseFloat(CARET_INDICATOR_SIZE_STYLE.width)).toBeGreaterThan(0.4);
            expect(Number.parseFloat(CARET_INDICATOR_SIZE_STYLE.width)).toBeLessThan(0.85);
            expect(Number.parseFloat(INDICATOR_SIZE_STYLE.width)).toBe(0.85);
        });
    });
