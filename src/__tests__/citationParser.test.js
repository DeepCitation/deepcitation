...
import { citationDataToCitation, extractCitationsFromMarkers, extractTrailingClaimText, extractVisibleText, getAllCitationsFromNumericResponse, getCitationMarkerIds, hasCitationData, parseCitationData, replaceCitationMarkers, stripCitations, stripClaimText, } from "../parsing/citationParser.js";
...
${CITATION_DATA_END_DELIMITER}`;
    ...
    it("handles null/undefined input", () => {
        const result = parseCitationData(null);
        expect(result.success).toBe(false);
        expect(result.error).toContain("Invalid input");
    });
...
describe("hasCitationData", () => {
    ...
    it("returns false for non-string input", () => {
        expect(hasCitationData(null)).toBe(false);
        expect(hasCitationData(123)).toBe(false);
    });
});
...
describe("replaceCitationMarkers with verifications", () => {
    const makeVerification = (status, citationNumber) => ({
        status,
        citation: {
            type: "document",
            citationNumber,
    ...
    it("resolves via citationMap key lookup when available", () => {
        ...
        if (!rawCitation)
            throw new Error("expected citationMap to have entry for key 1");
...
- Test result: POSITIVE [3]`;
    ...
    it("returns empty for empty/null input", () => {
        ...
        expect(Object.keys(extractCitationsFromMarkers(null)).length).toBe(0);
    });
...
describe("replaceCitationMarkers — cite: link format", () => {
    ...
    it("uses citationMap source_match when showSourceMatch is true", () => {
        const citationMap = new Map([[2, { id: 2, source_match: "Resolved Rate" }]]);
