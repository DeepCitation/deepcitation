...
jest.mock("../auth.js", () => ({
    ...
    generateNonce: () => "nonce",
    maskKey: (key) => key,
    sourceLabel: (source) => source.path ?? source.kind,
    resolveAuth: (...args) => mockResolveAuth(...args),
    writeCredentials: (...args) => mockWriteCredentials(...args),
    deleteCredentials: (...args) => mockDeleteCredentials(...args),
    openBrowser: (...args) => mockOpenBrowser(...args),
    startCallbackServer: (...args) => mockStartCallbackServer(...args),
}));
...
// ── Imports (after mocks) ─────────────────────────────────────────
import { canStartBrowserAuth, env, getAttachment, hydrate, inject, keygen, login, logout, openBillingDashboard, prepare, requireAuth, resolveBaseUrl, saveApiKey, status, verify, whoami, } from "../cli/commands.js";
...
describe("resolveBaseUrl", () => {
    ...
    afterEach(() => {
        if (origEnv === undefined)
            ...
            process.env.DC_LOGIN_URL = origEnv;
    });
...
describe("inject command", () => {
    ...
    it("exits on invalid --theme", () => {
        ...
        expect(() => inject(["--html", htmlPath, "--verify-response", verifyPath, "--theme", "purple"])).toThrow("process.exit(1)");
    });
    it("exits on invalid --indicator", () => {
        ...
        writeFileSync(htmlPath, "<html></html>");
        writeFileSync(verifyPath, JSON.stringify({ verifications: {} }));
        expect(() => inject(["--html", htmlPath, "--verify-response", verifyPath, "--indicator", "caret"])).toThrow("process.exit(1)");
    });
    it("applies --theme dark", async () => {
        ...
        writeFileSync(verifyPath, JSON.stringify({ verifications: {} }));
        await captureOutput(() => inject(["--html", htmlPath, "--verify-response", verifyPath, "--out", outPath, "--theme", "dark"]));
        const output = readFileSync(outPath, "utf-8");
    ...
    it("applies --indicator dot", async () => {
        ...
        writeFileSync(htmlPath, "<html><body></body></html>");
        writeFileSync(verifyPath, JSON.stringify({ verifications: {} }));
        await captureOutput(() => inject(["--html", htmlPath, "--verify-response", verifyPath, "--out", outPath, "--indicator", "dot"]));
        const output = readFileSync(outPath, "utf-8");
    ...
    it("injects key-map script when --key-map is provided", async () => {
        ...
        await captureOutput(() => inject(["--html", htmlPath, "--verify-response", verifyPath, "--key-map", keyMapPath, "--out", outPath]));
        const output = readFileSync(outPath, "utf-8");
    ...
    it("exits on invalid --key-map JSON", () => {
        ...
        expect(() => inject(["--html", htmlPath, "--verify-response", verifyPath, "--key-map", keyMapPath])).toThrow("process.exit(1)");
    });
    it("auto-fixes display-label when visible text differs from sourceMatch", async () => {
        ...
        writeFileSync(verifyPath, JSON.stringify({
            verifications: {
                abc123: { status: "found", citation: { sourceMatch: "completely different anchor text" } },
            },
        }));
        const { stderr } = await captureOutput(() => inject(["--html", htmlPath, "--verify-response", verifyPath, "--out", outPath]));
        const output = readFileSync(outPath, "utf-8");
...
describe("keygen command", () => {
    ...
    it("prints key mapping to stdout", async () => {
        ...
        writeFileSync(citPath, JSON.stringify({
            "my-label": {
...
describe("prepare command", () => {
    const fmtNetErr = (err) => (err instanceof Error ? err.message : String(err));
    beforeEach(() => {
        mockResolveAuth.mockReturnValue(makeAuth());
    });
    ...
    it("exits when file is not found", async () => {
        await expect(captureOutput(() => prepare(["/nonexistent/path/to/file.pdf"], fmtNetErr))).rejects.toThrow("process.exit(1)");
    });
    ...
    it("writes prepared JSON only when --out is provided", async () => {
        ...
        try {
            ...
            const { stdout } = await captureOutput(() => prepare(["https://example.com/article", "--out", outPath], fmtNetErr));
    ...
    it("prints cleaned prose when --format plain is used", async () => {
        ...
        try {
            ...
            const { stdout } = await captureOutput(() => prepare(["https://example.com/doc", "--format", "plain"], fmtNetErr));
    ...
    it("writes tagged text when --format txt --out is provided", async () => {
        ...
        try {
            ...
            const { stdout } = await captureOutput(() => prepare(["https://example.com/doc", "--format", "txt", "--out", outPath], fmtNetErr));
            expect(stdout).toBe(outPath);
            expect(readFileSync(outPath, "utf-8")).toBe(`<page_number_1_index_0><line id="1">Hello world.</line></page_number_1_index_0>`);
        }
        finally {
            process.chdir(origCwd);
        }
    ...
    it.each(["--text", "--txt", "--summary"])("rejects removed prepare flag %s", async (removedFlag) => {
        await expect(captureOutput(() => prepare(["https://example.com/doc", removedFlag], fmtNetErr))).rejects.toThrow("process.exit(1)");
    });
...
describe("verify command (--citations mode)", () => {
    ...
    it("exits on invalid --image-format", async () => {
        ...
        await expect(captureOutput(() => verify(["--citations", citPath, "--image-format", "gif"], fmtNetErr))).rejects.toThrow("process.exit(1)");
    });
    ...
    it("does not auto-promote a display label when it has no overlap with the anchor", async () => {
        ...
        try {
            ...
            const citationsArg = mockVerifyAttachment.mock.calls[0]?.[1];
...
describe("hydrateCitations", () => {
    ...
    it("fills source_context from a single lineId and pulls ±1 neighbor lines", () => {
        ...
        expect(citations[0].source_context).toBe("The Discount Rate is 80% of the lowest price per share. The Purchase Amount is the amount invested.");
    });
    it("concatenates text for multi-lineId citations and pulls ±1 neighbor lines", () => {
        ...
        expect(citations[0].source_context).toBe("The Discount Rate is 80% of the lowest price per share. The Purchase Amount is the amount invested. A Dissolution Event means a liquidation.");
    });
...
describe("hydrate CLI command", () => {
    it("fills source_context in draft file and writes output", async () => {
        ...
        writeFileSync(mdPath, `The [Discount Rate](cite:1) is 80%.
...
`);
        ...
        const result = JSON.parse(readFileSync(mdPath, "utf-8").split("<<<CITATION_DATA>>>")[1].split("<<<END_CITATION_DATA>>>")[0].trim());
        // Cited [1], expanded to [1, 2] by the neighbor-line widening.
        expect(result[0].source_context).toBe("The Discount Rate is 80% of the lowest price per share. The Purchase Amount is the amount invested.");
...
describe("login command — browser-auth gate", () => {
    ...
    afterEach(() => {
        ...
        if (origMsystem === undefined)
            delete process.env.MSYSTEM;
        else
            process.env.MSYSTEM = origMsystem;
    });
