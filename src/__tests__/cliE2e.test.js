...
import { cachedFixture, canRunE2e } from "./fixtures/fixtureCache.js";
// ── Setup ─────────────────────────────────────────────────────────
...
function getClient() {
    ...
    if (!apiKey)
        throw new Error("DEEPCITATION_API_KEY required for live API calls");
...
describe("E2E: prepare", () => {
    ...
    (canRun ? it : it.skip)("prepares a URL and returns attachmentId + metadata", async () => {
        const result = await cachedFixture("prepare-example-com", async () => {
            const dc = getClient();
...
describe("E2E: get attachment", () => {
    ...
    (canRun ? it : it.skip)("fetches attachment metadata by ID", async () => {
        ...
        const prepare = await cachedFixture("prepare-example-com", async () => {
            throw new Error("prepare fixture must exist first");
        });
        const result = await cachedFixture("get-attachment", async () => {
            const dc = getClient();
...
describe("E2E: verify citations", () => {
    ...
    (canRun ? it : it.skip)("verifies a citation against a prepared attachment", async () => {
        ...
        const result = await cachedFixture("verify-citations", async () => {
            const dc = getClient();
            ...
            const res = await dc.verifyAttachment(prepare.attachmentId, citations, { outputImageFormat: "avif" });
            return {
                verifications: Object.fromEntries(Object.entries(res.verifications).map(([k, v]) => [
                    k,
                    { status: v.status, citation: v.citation },
                ])),
            };
        });
        ...
        expect(["found", "partial_text_found", "found_source_match_only", "not_found", "found_on_other_page"]).toContain(firstVerification.status);
    });
