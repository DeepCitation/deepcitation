...
const VALID = sectionWithBlock(
  "Revenue grew **45%** [1] in Q4, beating **consensus** [2] estimates.",
  JSON.stringify({
    doc1: [
...
describe("lint", () => {
  ...
  function lintAndCatchExit(args: string[]): number {
    try {
      lint(args);
    } catch (err) {
  ...
  it("warns when k exceeds 4 words", () => {
    const content = sectionWithBlock(
      "Body **five word bold term here** [1].",
      JSON.stringify({
        doc1: [
          {
            n: 1,
  ...
  it("warns when intervening text sits between ** and [N]", () => {
    ...
    const content = sectionWithBlock(
      "The actual **reward** of the total rewards [1] is significant.",
      JSON.stringify({
