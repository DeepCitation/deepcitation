...
describe("retagEveryN", () => {
  ...
  it("every=2 tags even-numbered lines plus first and last", () => {
    ...
    for (const id of [1, 2, 4, 6, 8, 10]) expect(out).toContain(`<line id="${id}">`);
    for (const id of [3, 5, 7, 9]) expect(out).not.toContain(`<line id="${id}">`);
  ...
  it("every=3 tags multiples of 3 plus first and last", () => {
    ...
    for (const id of [1, 3, 6, 9, 10]) expect(out).toContain(`<line id="${id}">`);
    for (const id of [2, 4, 5, 7, 8]) expect(out).not.toContain(`<line id="${id}">`);
  });
