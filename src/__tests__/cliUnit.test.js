...
import { CLAUDE_COWORK_DOMAIN_HINT, checkForUpdate, extractApiKey, formatNetworkError, isValidApiKeyFormat, parseArgs, } from "../cli/cliUtils.js";
...
describe("formatNetworkError", () => {
    ...
    it("formats TimeoutError with structured human + marker output", () => {
        ...
        if (!markerLine)
            throw new Error("expected __DC_ERROR__ marker line in formatNetworkError output");
    ...
    it("redacts user:password@ from proxy URL in TimeoutError output", () => {
        const err = new TimeoutError("response_headers", 60023, "http://user:secret@proxy.example.com:3128", "api.deepcitation.com:443");
        const result = formatNetworkError(err, BASE_URL);
...
describe("checkForUpdate", () => {
    ...
    it("skips fetch when stamp is recent (within 24h)", async () => {
        ...
        readFileSync.mockReturnValue(String(Date.now() - 60_000));
    ...
    it("fetches when stamp is older than 24h", async () => {
        ...
        readFileSync.mockReturnValue(String(Date.now() - 25 * 60 * 60 * 1000));
        await checkForUpdate("0.3.10");
