...
function makeUrlErrorResponse(message, status = 400) {
    return {
        ok: false,
        status,
...
describe("prepareUrl() — URL failure scenarios", () => {
    ...
    describe("invalid URL format", () => {
        it("throws ValidationError when URL does not start with http:// or https://", async () => {
            mockFetch.mockResolvedValueOnce(makeUrlErrorResponse("Invalid URL format. URL must start with http:// or https://"));
    ...
    describe("file / content type problems", () => {
        ...
        it("throws ValidationError for unsupported content type at URL", async () => {
            mockFetch.mockResolvedValueOnce(makeUrlErrorResponse("Unsupported file type at URL. Accepted: PDF, Office documents, and images."));
    ...
    describe("service-level failures", () => {
        it("throws ServerError (retryable) when Office file conversion fails", async () => {
            mockFetch.mockResolvedValueOnce(makeUrlErrorResponse("Office file conversion failed. Please try again later.", 503));
