...
import type { ReviewUrlResponse } from "../../client/reviewUrl.types.js";
...
function makeReviewError(message: string, status = 400) {
  return {
    ok: false,
    status,
...
function makeReviewSuccess(overrides: Partial<ReviewUrlResponse> = {}): Response {
  const base: ReviewUrlResponse = {
...
describe("reviewUrl() — URL failure scenarios", () => {
  ...
  it("throws ValidationError for unsupported content type", async () => {
    mockFetch.mockResolvedValueOnce(
      makeReviewError("Unsupported file type at URL. Accepted: PDF, Office documents, and images."),
    );
