...
const verifiedVerification = { status: "found" };
const partialVerification = { status: "found_on_other_page" };
const missVerification = { status: "not_found" };
...
describe("Dot Indicator Variant", () => {
    ...
    describe("CitationComponent", () => {
        it("renders dot indicator for verified status", () => {
            const { container } = render(<CitationComponent citation={baseCitation} verification={verifiedVerification} indicatorVariant="dot"/>);
            // Dot indicators use rounded-full class (no SVG)
            const dots = container.querySelectorAll(".rounded-full");
            expect(dots.length).toBeGreaterThan(0);
        });
        it("renders dot indicator for partial match status", () => {
            const { container } = render(<CitationComponent citation={baseCitation} verification={partialVerification} indicatorVariant="dot"/>);
            const dot = container.querySelector(".bg-dc-partial");
            expect(dot).toBeInTheDocument();
        });
        it("renders dot indicator for miss status", () => {
            const { container } = render(<CitationComponent citation={baseCitation} verification={missVerification} indicatorVariant="dot"/>);
            const dot = container.querySelector(".bg-dc-destructive");
            expect(dot).toBeInTheDocument();
        });
        it("renders pulsing dot for pending status", () => {
            const { container } = render(<CitationComponent citation={baseCitation} verification={pendingVerification} indicatorVariant="dot"/>);
        ...
        it("renderIndicator overrides dot variant", () => {
            const customIndicator = (status) => status.isVerified ? <span data-testid="custom">Custom</span> : null;
            const { getByTestId } = render(<CitationComponent citation={baseCitation} verification={verifiedVerification} indicatorVariant="dot" renderIndicator={customIndicator}/>);
            expect(getByTestId("custom")).toBeInTheDocument();
        });
        it('indicatorVariant="none" hides dot indicator', () => {
            const { container } = render(<CitationComponent citation={baseCitation} verification={verifiedVerification} indicatorVariant="none"/>);
        ...
        it("dot indicator works with chip variant", () => {
            const { container } = render(<CitationComponent citation={baseCitation} verification={verifiedVerification} indicatorVariant="dot" variant="chip"/>);
