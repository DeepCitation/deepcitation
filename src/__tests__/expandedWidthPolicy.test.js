...
import { EXPANDED_POPOVER_MID_WIDTH, getExpandedPopoverWidth, getSummaryPopoverWidth, } from "../react/expandedWidthPolicy.js";
describe("expandedWidthPolicy", () => {
    ...
    it("returns image-derived width expression for known width", () => {
        const width = 623;
        expect(getExpandedPopoverWidth(width)).toBe(`max(${POPOVER_WIDTH_MIN_PX}px, min(${width + EXPANDED_IMAGE_SHELL_PX}px, calc(100dvw - 2rem)))`);
    });
    it("rounds fractional image widths for stable CSS output", () => {
        expect(getExpandedPopoverWidth(600.6)).toBe(`max(${POPOVER_WIDTH_MIN_PX}px, min(${Math.round(600.6) + EXPANDED_IMAGE_SHELL_PX}px, calc(100dvw - 2rem)))`);
    });
