...
describe("i18n unused keys", () => {
    ...
    it("has no new unused message keys (update KNOWN_UNUSED if intentional)", () => {
        ...
        if (newlyUnused.length > 0) {
            throw new Error(`${newlyUnused.length} new unused key(s) in defaultMessages. Either use them or add to KNOWN_UNUSED:\n` +
    ...
    it("has no stale KNOWN_UNUSED entries (remove if now used)", () => {
        ...
        if (stale.length > 0) {
            throw new Error(`${stale.length} KNOWN_UNUSED key(s) are now used — remove them from the allowlist:\n` +
                stale.map(k => `  "${k}"`).join("\n"));
        }
    });
    it("has no KNOWN_UNUSED entries missing from defaultMessages", () => {
        ...
        if (ghost.length > 0) {
            throw new Error(`${ghost.length} KNOWN_UNUSED key(s) no longer exist in defaultMessages — remove them from the allowlist:\n` +
                ghost.map(k => `  "${k}"`).join("\n"));
        }
    });
});
