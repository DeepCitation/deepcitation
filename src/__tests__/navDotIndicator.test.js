...
describe("nav-dot Indicator Variant", () => {
    ...
    it("renders the nav dot regardless of verification status (verified)", () => {
        const { container } = render(<CitationComponent citation={baseCitation} verification={verifiedVerification} indicatorVariant="nav-dot"/>);
    ...
    it("renders the nav dot regardless of verification status (miss)", () => {
        const { container } = render(<CitationComponent citation={baseCitation} verification={missVerification} indicatorVariant="nav-dot"/>);
        expect(container.querySelector('[data-dc-indicator="nav"]')).toBeInTheDocument();
    });
    it("renders the nav dot in superscript indicator-only mode", () => {
        const { container } = render(<CitationComponent citation={baseCitation} indicatorVariant="nav-dot" variant="superscript" content="indicator"/>);
        expect(container.querySelector('[data-dc-indicator="nav"]')).toBeInTheDocument();
    });
    it("renders the nav dot in footnote indicator-only mode", () => {
        const { container } = render(<CitationComponent citation={baseCitation} indicatorVariant="nav-dot" variant="footnote" content="indicator"/>);
        expect(container.querySelector('[data-dc-indicator="nav"]')).toBeInTheDocument();
    });
});
describe("Empty footnote marker fallback", () => {
    ...
    it("falls back to the citation number when superscript indicator-only has no status glyph", () => {
        const { container } = render(<CitationComponent citation={baseCitation} variant="superscript" content="indicator"/>);
