...
function makeLegacyResponse(overrides) {
    return {
        verifications: {
            key1: {
                status: "found",
...
describe("normalizeVerifyResponse", () => {
    ...
    it("uses first-writer-wins for duplicate attachmentIds", () => {
        ...
        const response = {
            verifications: {
                k1: { status: "found", attachmentId: "att-1", pageImages: PAGE_IMAGES },
                k2: { status: "found", attachmentId: "att-1", originalDownload: ORIGINAL_DL },
            },
        };
    ...
    it("skips verifications without attachmentId", () => {
        const response = {
            verifications: {
                k1: { status: "not_found" },
                k2: { status: "found", attachmentId: "att-1", pageImages: PAGE_IMAGES },
            },
        };
        const result = normalizeVerifyResponse(response);
    ...
    it("handles multiple different attachmentIds", () => {
        const response = {
            verifications: {
                k1: { status: "found", attachmentId: "att-1", pageImages: PAGE_IMAGES },
                k2: { status: "found", attachmentId: "att-2", originalDownload: ORIGINAL_DL },
            },
        };
        const result = normalizeVerifyResponse(response);
        expect(result.attachments?.["att-1"]).toEqual({ pageImages: PAGE_IMAGES });
    ...
    it("returns empty attachments map when no verifications have assets", () => {
        const response = {
            verifications: {
                k1: { status: "found", attachmentId: "att-1" },
            },
        };
        const result = normalizeVerifyResponse(response);
    ...
    it("hoists pageImagesStatus alongside pageImages", () => {
        ...
        expect(result.verifications.k1.pageImagesStatus).toBeUndefined();
    });
});
