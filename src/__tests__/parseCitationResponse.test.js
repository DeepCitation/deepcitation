...
describe("parseCitationResponse — numeric format", () => {
    const NUMERIC_RESPONSE = makeNumericResponse("The company reported strong growth [1]. Revenue increased significantly [2].", [
        {
            id: 1,
...
describe("parseCitationResponse — cite link format", () => {
    const CITE_LINK_RESPONSE = makeNumericResponse("The [Discount Rate](cite:1) is applied to the [Conversion Price](cite:2).", [
        {
            id: 1,
            attachment_id: "abc12345678901234567",
...
describe("parseCitationResponse — sourceMatch-only admission (issue-235)", () => {
    it("admits a citation that has sourceMatch but no sourceContext", () => {
        ...
        const response = makeNumericResponse("The patient has moderate impairment [3] and follows medication schedule [4].", [
            {
                id: 3,
