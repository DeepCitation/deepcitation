...
describe("promptCompression compress/decompress cycles", () => {
    ...
    const runCycle = (template) => {
        ...
        expect(decompressPromptIds(decompressed, prefixMap)).toBe(decompressed);
        const recompressed = compressPromptIds(decompressed, [fullId]);
...
describe("promptCompression ID handling", () => {
    ...
    it("handles IDs in JSON citation data", () => {
        ...
        const decompressed = decompressPromptIds(compressed, prefixMap);
        expect(decompressed).toBe(original);
        expect(decompressPromptIds(decompressed, prefixMap)).toBe(original);
    });
...
describe("promptCompression edge cases", () => {
    ...
    it("handles object input", () => {
        ...
        expect(compressed.content).toBe(`Reference: ${prefix}`);
        expect(compressed.id).toBe(prefix);
        const decompressed = decompressPromptIds(compressed, prefixMap);
    ...
    it("does not expand a prefix inside an ID that the model already returned uncompressed", () => {
        ...
        expect(decompressPromptIds(decompressPromptIds(modelOutput, prefixMap), prefixMap)).toBe(modelOutput);
    });
