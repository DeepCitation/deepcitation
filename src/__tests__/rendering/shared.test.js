...
const PARSED_INPUT = {
    ...
    format: "numeric",
    citations: {
        key1: { type: "document", attachmentId: "att1", pageNumber: 1, lineIds: [], sourceContext: "hello" },
        key2: {
            type: "url",
            attachmentId: "",
            pageNumber: 0,
...
describe("resolveSourceLabel", () => {
    it("uses sourceLabels for document citations", () => {
        const cws = {
            citation: { type: "document", attachmentId: "att1" },
            verification: null,
            citationNumber: 1,
        };
        expect(resolveSourceLabel(cws, { att1: "My Document" })).toBe("My Document");
    });
    it("falls back to Source N", () => {
        ...
        expect(resolveSourceLabel(cws, {})).toBe("Source 3");
    });
    it("uses title for URL citations when no label provided", () => {
        const cws = { citation: { type: "url", title: "Example Page" }, verification: null, citationNumber: 1 };
        expect(resolveSourceLabel(cws, {})).toBe("Example Page");
    });
});
// ── walkCitationSegments — cite link format ─────────────────────
const PARSED_CITE_LINK = {
...
describe("walkCitationSegments — cite link format", () => {
    ...
    it("handles mixed [N] and cite-link segments in same walk", () => {
        const mixed = {
            ...
            format: "numeric",
            citations: {
                key1: { type: "document", attachmentId: "a", pageNumber: 1, lineIds: [], sourceContext: "old" },
                key2: { type: "document", attachmentId: "b", pageNumber: 2, lineIds: [], sourceContext: "rate" },
            },
            markerMap: { 1: "key1", 2: "key2" },
        };
    ...
    it("does not emit cite-link marker as text when resolved", () => {
        const { segments } = walkCitationSegments(PARSED_CITE_LINK);
        const textSegments = segments.filter((s) => s.type === "text").map(s => s.value);
