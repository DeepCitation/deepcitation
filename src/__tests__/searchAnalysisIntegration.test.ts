...
function makeAttempt(overrides: Partial<SearchAttempt>): SearchAttempt {
  return {
...
describe("analyzeVerification integration", () => {
  ...
  it("partial match with low-trust variation", () => {
    const analysis = analyzeVerification(
      makeVerification({
  ...
  it("every SearchStatus maps to a defined outcome and colorScheme", () => {
    const allStatuses: SearchStatus[] = Object.keys(STATUS_MAP) as SearchStatus[];
