...
function makeUrlCitation(overrides) {
    return {
        type: "url",
...
function makeUrlFailureVerification(urlAccessStatus, errorMessage) {
    return {
        status: "not_found",
        url: {
            urlAccessStatus,
...
describe("URL Access Explanation in CitationComponent", () => {
    ...
    describe("blocked scenarios show amber explanation", () => {
        it("shows 'Login Required' for forbidden status", async () => {
            const { container, getByText } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("forbidden")}/>);
        ...
        it("shows 'Blocked by Site Protection' for generic blocked status", async () => {
            const { container, getByText } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("blocked", "Cloudflare challenge page detected")}/>);
            const trigger = container.querySelector("[data-citation-id]");
            await act(async () => {
                fireEvent.click(trigger);
            });
        ...
        it("infers paywall from error message keywords", async () => {
            const { container, getByText } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("blocked", "Paywall detected - subscription required")}/>);
            const trigger = container.querySelector("[data-citation-id]");
            await act(async () => {
                fireEvent.click(trigger);
            });
        ...
        it("infers login required from error message keywords", async () => {
            const { container, getByText } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("blocked", "Sign in page detected")}/>);
            const trigger = container.querySelector("[data-citation-id]");
            await act(async () => {
                fireEvent.click(trigger);
            });
        ...
        it("infers rate limit from error message with 429", async () => {
            const { container, getByText } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("blocked", "HTTP 429: Too Many Requests")}/>);
            const trigger = container.querySelector("[data-citation-id]");
            await act(async () => {
                fireEvent.click(trigger);
            });
        ...
        it("infers geo-restriction from error message keywords", async () => {
            const { container, getByText } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("blocked", "Content is only available in the UK")}/>);
            const trigger = container.querySelector("[data-citation-id]");
            await act(async () => {
                fireEvent.click(trigger);
            });
    ...
    describe("error scenarios show red explanation", () => {
        it("shows 'Page Not Found' for not_found status", async () => {
            const { container, getByText } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("not_found")}/>);
            const trigger = container.querySelector("[data-citation-id]");
            await act(async () => {
                fireEvent.click(trigger);
            });
        ...
        it("shows 'Server Error' for server_error status", async () => {
            const { container, getByText } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("server_error")}/>);
            const trigger = container.querySelector("[data-citation-id]");
            await act(async () => {
                fireEvent.click(trigger);
            });
        ...
        it("shows 'Connection Timed Out' for timeout status", async () => {
            const { container, getByText } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("timeout")}/>);
            const trigger = container.querySelector("[data-citation-id]");
            await act(async () => {
                fireEvent.click(trigger);
            });
        ...
        it("shows 'Network Error' for network_error status", async () => {
            const { container, getByText } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("network_error")}/>);
            const trigger = container.querySelector("[data-citation-id]");
            await act(async () => {
                fireEvent.click(trigger);
            });
    ...
    describe("custom error messages", () => {
        it("uses errorMessage as description when provided", async () => {
            const { container, getByText } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("server_error", "502 Bad Gateway from upstream proxy")}/>);
            const trigger = container.querySelector("[data-citation-id]");
            await act(async () => {
                fireEvent.click(trigger);
            });
    ...
    describe("non-error states show no explanation", () => {
        it("shows no explanation for accessible URL citation", async () => {
            const { container, queryByText } = render(<CitationComponent citation={makeUrlCitation()} verification={{
                    status: "found",
        ...
        it("shows no explanation for document citations", async () => {
            ...
            const { container, queryByText } = render(<CitationComponent citation={docCitation} verification={{
                    status: "not_found",
                    searchAttempts: [],
                }}/>);
            const trigger = container.querySelector("[data-citation-id]");
    ...
    describe("accessibility", () => {
        it("explanation banner has role=status and aria-label", async () => {
            const { container } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("forbidden")}/>);
            const trigger = container.querySelector("[data-citation-id]");
            await act(async () => {
                fireEvent.click(trigger);
            });
        ...
        it("amber banner has Warning aria-label prefix", async () => {
            const { container } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("blocked")}/>);
            const trigger = container.querySelector("[data-citation-id]");
            await act(async () => {
                fireEvent.click(trigger);
            });
        ...
        it("red banner has Error aria-label prefix", async () => {
            const { container } = render(<CitationComponent citation={makeUrlCitation()} verification={makeUrlFailureVerification("server_error")}/>);
            const trigger = container.querySelector("[data-citation-id]");
            await act(async () => {
                fireEvent.click(trigger);
            });
