...
const mockStartEvidenceViewTransition = mock((cb) => cb());
const mockStartEvidencePageExpandTransition = mock((cb) => cb());
const mockStartEvidencePageCollapseTransition = mock((cb) => cb());
...
import { usePopoverViewState } from "../react/hooks/usePopoverViewState";
...
describe("usePopoverViewState", () => {
    ...
    describe("onExpandedWidthChange", () => {
        ...
        it("clears width when source override is not an expanded state", () => {
            ...
            act(() => result.current.transition("expanded-keyhole"));
            act(() => result.current.onExpandedWidthChange(350));
            act(() => result.current.onExpandedWidthChange(null, "summary"));
            expect(result.current.expandedNaturalWidth).toBeNull();
        });
    ...
    describe("haptics", () => {
        it("fires expand haptic when going from summary to expanded-page", () => {
            const { result } = renderHook(() => usePopoverViewState(createConfig({ experimentalHaptics: true, isMobile: true })));
            act(() => result.current.transition("expanded-page"));
        ...
        it("does not fire haptics when experimentalHaptics is false", () => {
            const { result } = renderHook(() => usePopoverViewState(createConfig({ experimentalHaptics: false, isMobile: true })));
            act(() => result.current.transition("expanded-page"));
