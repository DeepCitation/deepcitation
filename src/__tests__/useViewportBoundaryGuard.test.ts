...
function makeRefs(el: HTMLElement): {
...
type HookProps = { popoverViewState: PopoverViewState; sideOffset: number | undefined };

function renderGuard(el: HTMLElement, initialProps: HookProps) {
  const { popoverContentRef, triggerRef } = makeRefs(el);
  return renderHook(
    ({ popoverViewState, sideOffset }: HookProps) =>
      useViewportBoundaryGuard(true, popoverViewState, popoverContentRef, triggerRef, sideOffset),
    { initialProps },
  );
}
