...
function createMockWrapper(container) {
    const el = document.createElement("div");
...
describe("useWheelZoom", () => {
    ...
    it("returns expected shape", () => {
        ...
        const { result } = renderHook(() => useWheelZoom({
            enabled: true,
            sensitivity: 0.005,
            containerRef: { current: container },
            wrapperRef: { current: wrapper },
            zoom: 1.0,
            clampZoomRaw: (z) => Math.max(0.5, Math.min(3.0, z)),
            clampZoom: (z) => Math.round(Math.max(0.5, Math.min(3.0, z)) * 100) / 100,
            onZoomCommit: () => { },
        }));
    ...
    it("commits zoom after 150ms debounce", async () => {
        const container = createMockContainer();
        const wrapper = createMockWrapper(container);
        const onZoomCommit = mock(() => { });
        renderHook(() => useWheelZoom({
            enabled: true,
            sensitivity: 0.005,
            containerRef: { current: container },
            wrapperRef: { current: wrapper },
