...
function validateCitationStructure(citations) {
    const errors = [];
    const warnings = [];
    ...
    for (const c of citations) {
        ...
        // Required fields
        if (!c.attachment_id)
            errors.push(`${prefix} missing attachment_id`);
        if (!c.source_context)
            errors.push(`${prefix} missing source_context`);
        if (!c.source_match)
            errors.push(`${prefix} missing source_match`);
        if (!c.page_id)
            errors.push(`${prefix} missing page_id`);
        if (!c.line_ids || c.line_ids.length === 0)
            errors.push(`${prefix} missing or empty line_ids`);
        if (!c.reasoning)
            warnings.push(`${prefix} missing reasoning`);
        // page_id format
        ...
        if (c.source_match && c.source_context) {
            if (!c.source_context.includes(c.source_match)) {
                errors.push(`${prefix} source_match not a verbatim substring of source_context: ` +
...
function validateMarkdownAlignment(markdown, citations) {
    const errors = [];
    const warnings = [];
...
function validateVerbatimAgainstPrepare(citations, prepareOutputs) {
    const errors = [];
    const warnings = [];
    for (const c of citations) {
        if (!c.attachment_id || !c.source_context)
            continue;
        ...
        if (!deepText.includes(c.source_context)) {
            errors.push(`[${c.id}] source_context NOT found verbatim in source deepTextPages: ` +
...
function loadPrepareOutputs(dir) {
    const outputs = new Map();
    if (!existsSync(dir))
        return outputs;
    for (const file of readdirSync(dir)) {
        if (!file.startsWith("prepare-") || !file.endsWith(".json"))
            continue;
        try {
            const data = JSON.parse(readFileSync(resolve(dir, file), "utf-8"));
            if (data.attachmentId) {
                if (Array.isArray(data.deepTextPages) &&
                    data.deepTextPages.every((page) => typeof page === "string")) {
...
if (DRAFT_FILE || DRAFT_DIR) {
    ...
    const prepareOutputs = PREPARE_DIR ? loadPrepareOutputs(resolve(PREPARE_DIR)) : new Map();
