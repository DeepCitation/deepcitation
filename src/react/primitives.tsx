  ...
  (
    ...
    if (status.isPending && shouldShow("pending") && pendingIndicator) {
      return (
        <span
          ref={ref}
          className={classNames(baseClasses, "text-slate-500 dark:text-slate-400", className)}
...
export const CitationPage = forwardRef<HTMLSpanElement, CitationPageProps>(
  ({ className, prefix = "", ...props }, ref) => {
    ...
    return (
      <span ref={ref} className={classNames("text-xs text-slate-500 dark:text-slate-400", className)} {...props}>
        {prefix}
        {citation.pageNumber}
      </span>
    );
