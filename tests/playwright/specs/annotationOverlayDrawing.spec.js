...
test.describe("Annotation Overlay Drawing — elements", () => {
    test("renders spotlight, brackets, and anchor highlight with correct structure", async ({ mount, page, }) => {
        await mount(<div style={{ position: "relative", width: `${IMAGE_W}px`, height: `${IMAGE_H}px` }}>
        <CitationAnnotationOverlay sourceContextDeepItem={PHRASE_ITEM} renderScale={RENDER_SCALE} imageNaturalWidth={IMAGE_W} imageNaturalHeight={IMAGE_H} sourceMatchDeepItems={[ANCHOR_ITEM]} sourceMatch="Functional status" sourceContext="Functional status: He is at baseline, no assistance needed, independent ADLs"/>
      </div>);
    ...
    test("returns null (no overlay) when renderScale is zero (invalid geometry)", async ({ mount, page, }) => {
        await mount(<div style={{ position: "relative", width: `${IMAGE_W}px`, height: `${IMAGE_H}px` }}>
        <CitationAnnotationOverlay sourceContextDeepItem={PHRASE_ITEM} renderScale={{ x: 0, y: 0 }} imageNaturalWidth={IMAGE_W} imageNaturalHeight={IMAGE_H}/>
      </div>);
...
test.describe("Annotation Overlay Drawing — bracket colors", () => {
    test("brackets use green (verified) color by default (no highlightColor prop)", async ({ mount, page }) => {
        await mount(<div style={{ position: "relative", width: `${IMAGE_W}px`, height: `${IMAGE_H}px` }}>
        <CitationAnnotationOverlay sourceContextDeepItem={PHRASE_ITEM} renderScale={RENDER_SCALE} imageNaturalWidth={IMAGE_W} imageNaturalHeight={IMAGE_H}/>
      </div>);
        const leftBracket = page.locator("[data-dc-bracket-left]");
        await expect(leftBracket).toBeVisible();
        const borderColor = await leftBracket.evaluate(el => getComputedStyle(el).borderLeftColor);
    ...
    test("brackets use green (verified) color when highlightColor='green'", async ({ mount, page }) => {
        await mount(<div style={{ position: "relative", width: `${IMAGE_W}px`, height: `${IMAGE_H}px` }}>
        <CitationAnnotationOverlay sourceContextDeepItem={PHRASE_ITEM} renderScale={RENDER_SCALE} imageNaturalWidth={IMAGE_W} imageNaturalHeight={IMAGE_H} highlightColor="green"/>
      </div>);
        const leftBracket = page.locator("[data-dc-bracket-left]");
    ...
    test("brackets use amber color when highlightColor='amber'", async ({ mount, page }) => {
        await mount(<div style={{ position: "relative", width: `${IMAGE_W}px`, height: `${IMAGE_H}px` }}>
        <CitationAnnotationOverlay sourceContextDeepItem={PHRASE_ITEM} renderScale={RENDER_SCALE} imageNaturalWidth={IMAGE_W} imageNaturalHeight={IMAGE_H} highlightColor="amber"/>
      </div>);
        const leftBracket = page.locator("[data-dc-bracket-left]");
        const rightBracket = page.locator("[data-dc-bracket-right]");
        const leftColor = await leftBracket.evaluate(el => getComputedStyle(el).borderLeftColor);
...
test.describe("Annotation Overlay Drawing — bracket geometry", () => {
    ...
    test("bracket width matches getBracketWidth(heightPx) = 4px for 20px height", async ({ mount, page, }) => {
        await mount(<div style={{ position: "relative", width: `${IMAGE_W}px`, height: `${IMAGE_H}px` }}>
        <CitationAnnotationOverlay sourceContextDeepItem={PHRASE_ITEM} renderScale={RENDER_SCALE} imageNaturalWidth={IMAGE_W} imageNaturalHeight={IMAGE_H}/>
      </div>);
        const leftBracket = page.locator("[data-dc-bracket-left]");
    ...
    test("bracket width scales with item height (larger item → wider bracket, capped at 12px)", async ({ mount, page, }) => {
        // Height=80 → getBracketWidth(80) = max(4, min(80*0.2, 12)) = max(4, min(16, 12)) = 12
        const tallItem = { ...PHRASE_ITEM, height: 80 };
        await mount(<div style={{ position: "relative", width: `${IMAGE_W}px`, height: `${IMAGE_H}px` }}>
        <CitationAnnotationOverlay sourceContextDeepItem={tallItem} renderScale={RENDER_SCALE} imageNaturalWidth={IMAGE_W} imageNaturalHeight={IMAGE_H}/>
      </div>);
        const leftBracket = page.locator("[data-dc-bracket-left]");
        const width = await leftBracket.evaluate(el => getComputedStyle(el).width);
...
test.describe("Annotation Overlay Drawing — spotlight", () => {
    test("spotlight box-shadow uses OVERLAY_COLOR (rgba(26, 26, 26, 0.25))", async ({ mount, page, }) => {
        await mount(<div style={{ position: "relative", width: `${IMAGE_W}px`, height: `${IMAGE_H}px` }}>
        <CitationAnnotationOverlay sourceContextDeepItem={PHRASE_ITEM} renderScale={RENDER_SCALE} imageNaturalWidth={IMAGE_W} imageNaturalHeight={IMAGE_H}/>
      </div>);
    ...
    test("spotlight rect is larger than bracket rect by SPOTLIGHT_PADDING on each side", async ({ mount, page, }) => {
        await mount(<div style={{ position: "relative", width: `${IMAGE_W}px`, height: `${IMAGE_H}px` }}>
        <CitationAnnotationOverlay sourceContextDeepItem={PHRASE_ITEM} renderScale={RENDER_SCALE} imageNaturalWidth={IMAGE_W} imageNaturalHeight={IMAGE_H}/>
      </div>);
        const spotlight = page.locator("[data-dc-spotlight]");
    ...
    test("brackets use BOX_PADDING (smaller offset from text rect)", async ({ mount, page, }) => {
        await mount(<div style={{ position: "relative", width: `${IMAGE_W}px`, height: `${IMAGE_H}px` }}>
        <CitationAnnotationOverlay sourceContextDeepItem={PHRASE_ITEM} renderScale={RENDER_SCALE} imageNaturalWidth={IMAGE_W} imageNaturalHeight={IMAGE_H}/>
      </div>);
        const leftBracket = page.locator("[data-dc-bracket-left]");
...
test.describe("Annotation Overlay Drawing — anchor highlight", () => {
    test("anchor highlight renders with correct background color", async ({ mount, page }) => {
        ...
        const bgColor = await anchorHighlight.evaluate(el => getComputedStyle(el).backgroundColor);
    ...
    test("anchor highlight shown when sourceMatch and sourceContext are identical text", async ({ mount, page, }) => {
        const sameTextAnchor = {
            ...ANCHOR_ITEM,
            text: PHRASE_ITEM.text, // same text as phrase
        };
        await mount(<div style={{ position: "relative", width: `${IMAGE_W}px`, height: `${IMAGE_H}px` }}>
        <CitationAnnotationOverlay sourceContextDeepItem={PHRASE_ITEM} renderScale={RENDER_SCALE} imageNaturalWidth={IMAGE_W} imageNaturalHeight={IMAGE_H} sourceMatchDeepItems={[sameTextAnchor]} sourceMatch="Functional status" sourceContext="Functional status" // same as sourceMatch — still shown
        />
      </div>);
    ...
    test("anchor highlight shown when word difference < 2 (3 words vs 2 words)", async ({ mount, page, }) => {
        const shortAnchor = { ...ANCHOR_ITEM, text: "Functional status is" };
        await mount(<div style={{ position: "relative", width: `${IMAGE_W}px`, height: `${IMAGE_H}px` }}>
        <CitationAnnotationOverlay sourceContextDeepItem={PHRASE_ITEM} renderScale={RENDER_SCALE} imageNaturalWidth={IMAGE_W} imageNaturalHeight={IMAGE_H} sourceMatchDeepItems={[shortAnchor]} sourceMatch="Functional status" sourceContext="Functional status is" // 3 words vs 2 — still shown
        />
      </div>);
        // Always show anchor highlight when both sourceMatch and sourceContext are non-empty
        await expect(page.locator("[data-dc-anchor-highlight]")).toHaveCount(1);
    });
    test("anchor highlight hidden when no sourceMatchDeepItems is provided", async ({ mount, page, }) => {
        await mount(<div style={{ position: "relative", width: `${IMAGE_W}px`, height: `${IMAGE_H}px` }}>
        <CitationAnnotationOverlay sourceContextDeepItem={PHRASE_ITEM} renderScale={RENDER_SCALE} imageNaturalWidth={IMAGE_W} imageNaturalHeight={IMAGE_H} sourceMatch="Functional status" sourceContext="Functional status: He is at baseline, no assistance needed, independent ADLs"/>
      </div>);
