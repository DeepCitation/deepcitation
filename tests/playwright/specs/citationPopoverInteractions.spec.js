...
test.describe("Citation Popover - Click-to-Close Behavior", () => {
    ...
    test("bottom-edge keyhole expand should not oscillate vertically", async ({ mount, page }) => {
        await mount(<div style={{ paddingTop: "430px", paddingLeft: "80px" }}>
        ...
        const samples = await page.evaluate(async () => {
            ...
            await new Promise(resolve => {
                const tick = () => {
                    ...
                    const inlineFrame = dialog.querySelector("[data-dc-inline-expanded] > div");
        ...
        for (const dy of samples.guardDy) {
            ...
            if (prevGuardSign !== 0 && sign !== 0 && sign !== prevGuardSign)
                guardDyReversals++;
            if (sign !== 0)
                prevGuardSign = sign;
        }
    ...
    test("center keyhole click should not left-snap at same width or spike quote gap", async ({ mount, page }) => {
        await mount(<div style={{ paddingTop: "80px", paddingLeft: "220px" }}>
        <CitationComponent citation={baseCitation} verification={tallVerification} pageImagesByAttachmentId={tallPageImagesByAttachmentId}/>
      </div>);
        const citation = page.locator("[data-citation-id]");
        await citation.click();
        ...
        const samples = await page.evaluate(async () => {
            ...
            await new Promise(resolve => {
                const tick = () => {
                    ...
                    left.push(rect.left);
                    width.push(rect.width);
                    const quoteEl = Array.from(dialog.querySelectorAll("div")).find(el => el.textContent?.includes("The company reported 25% revenue growth in Q4"));
                    const keyhole = dialog.querySelector("[data-dc-inline-expanded]");
                    if (quoteEl && keyhole) {
    ...
    test("expanded-page roundtrip should not left-snap or show transient right gap", async ({ mount, page }) => {
        await mount(<div style={{ paddingTop: "120px", paddingLeft: "120px" }}>
        <CitationComponent citation={baseCitation} verification={tallVerification} pageImagesByAttachmentId={tallPageImagesByAttachmentId}/>
      </div>);
        const citation = page.locator("[data-citation-id]");
        await citation.click();
        ...
        const pageExpandSamples = await page.evaluate(async () => {
            ...
            await new Promise(resolve => {
                const tick = () => {
                    const visibleInline = Array.from(dialog.querySelectorAll("[data-dc-inline-expanded]")).find(el => el.offsetParent !== null);
                    if (visibleInline) {
                        const frame = visibleInline.querySelector(":scope > div");
        ...
        const minPageExpandOpacity = pageExpandSamples.inlineOpacity.length > 0 ? Math.min(...pageExpandSamples.inlineOpacity) : 1;
        ...
        const pageCollapseSamples = await page.evaluate(async () => {
            const dialog = document.querySelector("[role='dialog']");
            if (!dialog)
                return { maxTransitionSeconds: 0 };
            const parseMaxTransitionSeconds = (value) => {
                ...
                const tick = () => {
                    ...
                    if (visibleInline) {
                        ...
                        if (frame) {
                            ...
                            if (duration > maxTransitionSeconds)
                                maxTransitionSeconds = duration;
                        }
                    }
...
test.describe("Citation Popover - Mobile/Touch Behavior", () => {
    ...
    test("tap outside closes popover", async ({ mount, page }) => {
        await mount(<div style={{ width: "375px", height: "667px", position: "relative" }}>
