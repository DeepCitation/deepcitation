...
async function enableDebugStore(page) {
    await page.evaluate(() => {
...
async function readGhostSnapshot(page) {
    return page.evaluate(() => {
        const api = window.__dcAnimationDebug;
        const raw = api.snapshot().lastGhostRects;
        if (!raw)
            return null;
        const toPlain = (r) => r ? { x: r.x, y: r.y, width: r.width, height: r.height } : null;
        return {
...
async function waitForGhostDirection(page, direction, timeoutMs = 4000) {
...
async function expandToPage(_page, popover) {
    const expandButton = popover.getByLabel(/Expand to full page/).first();
    await expect(expandButton).toBeVisible();
    await expandButton.click();
}
async function collapseBackToSummary(page) {
...
async function expandToKeyhole(popover) {
...
test.describe("Anchor handoff: ghost ↔ keyhole (AsymmetricAnchorCitation)", () => {
    ...
    test("keyhole sourceAnchorX is textItem-derived (not clamped sourceContextDeepItem)", async ({ mount, page, }) => {
        await mount(<AsymmetricAnchorCitation />);
        await openSummaryPopover(page);
        const anchorX = await page
            .locator("[data-dc-source-anchor-x]")
            .first()
            .evaluate(el => Number.parseFloat(el.dataset.dcSourceAnchorX ?? "1"));
        ...
        expect(anchorX, `sourceAnchorX=${anchorX.toFixed(4)} — expected ~0.825 (textItem-based), got ${anchorX >= 0.95 ? "clamped sourceContextDeepItem" : "unknown"} value`).toBeGreaterThan(0.7);
    ...
    test("ghost anchor at collapse-end is at the annotation center, not the right edge", async ({ mount, page, }) => {
        await mount(<AsymmetricAnchorCitation />);
        ...
        expect(ghostAnchorX, `ghost anchor at ${ghostAnchorX.toFixed(0)} px is at or past keyholeRight=${keyholeRight.toFixed(0)} px — anchor is clamped to the right edge`).toBeLessThan(keyholeRight - 20);
        ...
        expect(Math.abs(ghostAnchorX - expectedAnchorX), `ghost anchor (${ghostAnchorX.toFixed(0)}) should be within 15 px of expected (${expectedAnchorX.toFixed(0)})`).toBeLessThanOrEqual(15);
    });
    ...
    test.describe("narrow viewport (800px) — forces image overflow in expanded-keyhole", () => {
        ...
        test("collapse ghost from expanded-page → expanded-keyhole: anchorInGhostX within ghost bounds (WideEvidenceCitation)", async ({ mount, page, }) => {
            ...
            expect(anchorInGhostX, `anchorInGhostX (${anchorInGhostX.toFixed(0)}) must be ≤ containerWidth (${containerWidth.toFixed(0)}). ` +
                `Without the scroll fix it would be ≈ 1040 (0.867 × 1200px image), well past the ghost's right edge.`).toBeLessThanOrEqual(containerWidth);
            // Secondary: ghost did not start wildly off-screen to the left.
            expect(collapse.source.x, `ghost source.x should not be further off-screen than one ghost-width (>= -containerWidth)`).toBeGreaterThanOrEqual(-containerWidth);
        });
    ...
    test.describe("narrow viewport (800px) — expand from expanded-keyhole without spotlight", () => {
        test.use({ viewport: { width: 800, height: 900 } });
        test("expand ghost from expanded-keyhole → page: anchorInGhostX within ghost bounds (WideEvidenceCitation)", async ({ mount, page, }) => {
            await mount(<WideEvidenceCitation />);
            await enableDebugStore(page);
            const popover = await openSummaryPopover(page);
            await expandToKeyhole(popover);
