 ...
 */
const makeTestImage = (width, height, color = "#cccccc") => {
...
function makeVerification(page, status = "found") {
    return {
        status,
        attachmentId,
        document: {
...
function makeItem(page, key, phrase, anchor) {
    return {
        citationKey: key,
...
function makeEscapeCascadeGroups() {
    return [
        {
...
function getItemWrapper(dialog, key) {
 ...
 */
async function expandToLevel3ViaPagePill(dialog, pageNumber, citationKey) {
...
test.describe("Drawer - Page Pill Highlighting", () => {
    test("page pills render in header for multi-page citations", async ({ mount, page }) => {
        await mount(<DrawerInteractionHarness groups={makeTwoPageGroups()} pageImagesByAttachmentId={{ [attachmentId]: makePageImages([3, 7]) }}/>);
...
test.describe("Drawer - Escape Cascade", () => {
    test("Escape from Level 1 (no expansion) closes the drawer", async ({ mount, page }) => {
        await mount(<DrawerInteractionHarness groups={makeEscapeCascadeGroups()} pageImagesByAttachmentId={{ [attachmentId]: makePageImages([1, 5]) }}/>);
        const dialog = page.locator("[role='dialog']");
        await expect(dialog).toBeVisible({ timeout: 5000 });
