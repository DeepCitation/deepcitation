...
const URL_CITATION = [
    {
        ...
        citation: {
            ...
            sourceMatch: "latest results",
            sourceContext: "According to the latest results published online.",
        },
        verification: { status: "found" },
    },
];
const TWO_DOC_SOURCES = [
    {
        citationKey: "m1",
        citation: {
            type: "document",
            attachmentId: "att-first",
            sourceMatch: "first claim",
            sourceContext: "This is the first claim from the report.",
            pageNumber: 1,
        },
    ...
    },
    {
        ...
        citation: {
            ...
            sourceMatch: "second claim",
            sourceContext: "This is a different claim from another file.",
            pageNumber: 7,
        },
 ...
 */
async function getTriggerLabelText(page) {
 ...
 */
async function openDrawerAndGetHeadingText(page) {
    const trigger = page.locator('[data-testid="citation-drawer-trigger"]');
    await trigger.click();
...
test.describe("Drawer ↔ Trigger source label consistency", () => {
    ...
    test("document citation with sourceLabelMap: labels match", async ({ mount, page }) => {
        await mount(<TriggerAndDrawer citations={DOC_CITATION} sourceLabelMap={{ "att-abc-123": "Q4 Financial Report" }}/>);
        const triggerLabel = await getTriggerLabelText(page);
        const drawerHeading = await openDrawerAndGetHeadingText(page);
    ...
    test("url citation with sourceLabelMap: labels match", async ({ mount, page }) => {
        await mount(<TriggerAndDrawer citations={URL_CITATION} sourceLabelMap={{ "https://blog.example.com/post/1": "Engineering Blog" }}/>);
        const triggerLabel = await getTriggerLabelText(page);
        const drawerHeading = await openDrawerAndGetHeadingText(page);
    ...
    test("multi-source with sourceLabelMap: resolved name matches", async ({ mount, page }) => {
        await mount(<TriggerAndDrawer citations={TWO_DOC_SOURCES} sourceLabelMap={{ "att-first": "Annual Report 2024" }}/>);
        const triggerLabel = await getTriggerLabelText(page);
        const drawerHeading = await openDrawerAndGetHeadingText(page);
        ...
        const citations = [
            ...DOC_CITATION,
            {
                ...
                citation: {
                    type: "document",
                    attachmentId: "att-abc-123",
                    sourceMatch: "net income doubled",
                    sourceContext: "Net income doubled compared to the prior year.",
                    pageNumber: 5,
                },
                verification: { status: "found", label: "att-abc-123.pdf" },
            },
        ];
        await mount(<TriggerAndDrawer citations={citations} sourceLabelMap={{ "att-abc-123": "Q4 Financial Report" }}/>);
        const triggerLabel = await getTriggerLabelText(page);
        expect(triggerLabel).toBe("Q4 Financial Report");
