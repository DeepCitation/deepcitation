...
async function captureExpandCollapseCycle(page) {
...
test.describe("Ghost path symmetry (asymmetric anchor fixture)", () => {
    ...
    test("fixture resolves sourceAnchorX ≠ 0.5 (precondition for the symmetry bugs)", async ({ mount, page, }) => {
        await mount(<AsymmetricAnchorCitation />);
        const popover = await openSummaryPopover(page);
        const keyhole = popover.locator("[data-dc-keyhole]").first();
        await expect(keyhole).toBeVisible();
        const anchorX = await keyhole.evaluate(el => Number.parseFloat(el.dataset.dcSourceAnchorX ?? ""));
        expect(anchorX).toBeGreaterThan(0.7); // ~0.825 by fixture construction
        expect(anchorX).toBeLessThan(0.95);
    });
    test("collapse-end matches expand-start for the ghost (anchor-symmetry bug)", async ({ mount, page, }) => {
        await mount(<AsymmetricAnchorCitation />);
    ...
    test("collapse-start matches expand-end at the spotlight (handoff bug)", async ({ mount, page, }) => {
        await mount(<AsymmetricAnchorCitation />);
        const { expand, collapse } = await captureExpandCollapseCycle(page);
    ...
    test("ghost never travels past its endpoints horizontally during collapse", async ({ mount, page, }) => {
        await mount(<AsymmetricAnchorCitation />);
    ...
    test.skip("expand and collapse trace time-reversed paths (within tolerance)", async ({ mount, page, }) => {
        await mount(<AsymmetricAnchorCitation />);
        const { expand, collapse } = await captureExpandCollapseCycle(page);
