 ...
 */
async function readKeyholeGeometry(page) {
    return page.evaluate(() => {
        const el = document.querySelector("[data-dc-keyhole]");
        if (!el)
            return null;
        ...
        if (!Number.isFinite(sax) || !Number.isFinite(say))
            return null;
 ...
 */
async function setKeyholeScrollLeft(page, scrollLeft) {
 ...
 */
async function captureCycleAtPan(page, popover, targetScrollLeft) {
    ...
    if (!pre)
        throw new Error("readKeyholeGeometry(pre) returned null after pan");
    ...
    if (!post)
        throw new Error("readKeyholeGeometry(post) returned null");
...
test.describe("Keyhole pan ↔ ghost endpoint alignment", () => {
    ...
    test("fixture gives a horizontally-scrollable keyhole with off-center anchor", async ({ mount, page, }) => {
    ...
    test("[diagnostic] expand works on this fixture without any prior scroll manipulation", async ({ mount, page, }) => {
    ...
    // -------------------------------------------------------------------------
    test("expand ghost starts at anchor's viewport position when anchor panned LEFT", async ({ mount, page, }) => {
        await mount(<PannedKeyholeCitation />);
        ...
        const { pre, expand } = await captureCycleAtPan(page, popover, anchorImageX - targetVisibleAnchor);
    ...
    test("expand ghost starts at anchor's viewport position when anchor panned RIGHT", async ({ mount, page, }) => {
        await mount(<PannedKeyholeCitation />);
        const popover = await openSummaryPopover(page);
        await expect(popover.locator("[data-dc-keyhole]").first()).toBeVisible();
        const geo0 = (await readKeyholeGeometry(page));
    ...
    // -------------------------------------------------------------------------
    test("collapse ghost lands at anchor's viewport position when anchor panned LEFT", async ({ mount, page, }) => {
        await mount(<PannedKeyholeCitation />);
        const popover = await openSummaryPopover(page);
        await expect(popover.locator("[data-dc-keyhole]").first()).toBeVisible();
        const geo0 = (await readKeyholeGeometry(page));
        const anchorImageX = geo0.sourceAnchorX * geo0.imageWidth;
        const { post, collapse } = await captureCycleAtPan(page, popover, anchorImageX - geo0.stripWidth * 0.2);
    ...
    test("collapse ghost lands at anchor's viewport position when anchor panned RIGHT", async ({ mount, page, }) => {
        await mount(<PannedKeyholeCitation />);
        const popover = await openSummaryPopover(page);
        await expect(popover.locator("[data-dc-keyhole]").first()).toBeVisible();
        const geo0 = (await readKeyholeGeometry(page));
        const anchorImageX = geo0.sourceAnchorX * geo0.imageWidth;
        const { post, collapse } = await captureCycleAtPan(page, popover, anchorImageX - geo0.stripWidth * 0.8);
        expect(collapse.target).not.toBeNull();
        const endAnchor = anchorViewportOf(collapse, collapse.target);
        expect(endAnchor.x).toBeGreaterThan(post.anchorViewportX - 3);
        expect(endAnchor.x).toBeLessThan(post.anchorViewportX + 3);
    ...
    test("ghost anchor handoff on collapse leaves no gap vs real keyhole anchor", async ({ mount, page, }) => {
        ...
        const geo0 = (await readKeyholeGeometry(page));
        const anchorImageX = geo0.sourceAnchorX * geo0.imageWidth;
        const { post, collapse } = await captureCycleAtPan(page, popover, anchorImageX - geo0.stripWidth * 0.25);
