...
const verificationWithTallImage = {
    status: "found",
    attachmentId: "att-1",
...
async function expandToFullPage(page) {
...
async function expectPopoverInViewport(page, popover, tolerance = 2) {
...
async function dispatchCtrlWheel(locator, clientX, clientY, deltaY) {
...
test.describe("Expanded-Page Basics", () => {
    test("image renders at a width that fills available space, not shrunk by height constraint", async ({ mount, page, }) => {
        ...
        const renderedWidth = await img.evaluate(el => el.getBoundingClientRect().width);
    ...
    test("expanded page surface uses contrasted canvas background in light mode", async ({ mount, page }) => {
        ...
        const backgroundColor = await expandedView.evaluate(el => window.getComputedStyle(el).backgroundColor);
    ...
    test("scroll container has usable height, not collapsed to content", async ({ mount, page }) => {
        ...
        const { expandedView } = await expandToFullPage(page);
        const containerHeight = await expandedView.evaluate(el => el.clientHeight);
...
test.describe("Expanded-Page Viewport Containment", () => {
    ...
    test("desktop: trigger near top — popover stays in viewport", async ({ mount, page }) => {
        await mount(<div style={{ paddingTop: "20px", paddingLeft: "100px" }}>
        <CitationComponent citation={baseCitation} verification={verificationWithTallImage} pageImagesByAttachmentId={pageImagesByAttachmentId}/>
      </div>);
    ...
    test("desktop: trigger in center — popover stays in viewport", async ({ mount, page }) => {
        await mount(<div style={{ paddingTop: "300px", paddingLeft: "100px" }}>
        <CitationComponent citation={baseCitation} verification={verificationWithTallImage} pageImagesByAttachmentId={pageImagesByAttachmentId}/>
      </div>);
        const { popoverContent } = await expandToFullPage(page);
        await expectPopoverInViewport(page, popoverContent);
    ...
    test("desktop: trigger near bottom — popover stays in viewport", async ({ mount, page }) => {
        await mount(<div style={{ paddingTop: "600px", paddingLeft: "100px" }}>
        <CitationComponent citation={baseCitation} verification={verificationWithTallImage} pageImagesByAttachmentId={pageImagesByAttachmentId}/>
      </div>);
        const { popoverContent } = await expandToFullPage(page);
        await expectPopoverInViewport(page, popoverContent);
    ...
    test("desktop: scroll-lock layout shift re-clamps expanded popover near left edge", async ({ mount, page }) => {
        await mount(<div style={{ position: "relative", height: "2600px" }}>
    ...
    test.describe("Tablet (768×1024)", () => {
        ...
        test("tablet: trigger near top — popover stays in viewport", async ({ mount, page }) => {
            await mount(<div style={{ paddingTop: "20px", paddingLeft: "50px" }}>
          <CitationComponent citation={baseCitation} verification={verificationWithTallImage} pageImagesByAttachmentId={pageImagesByAttachmentId}/>
        </div>);
            const { popoverContent } = await expandToFullPage(page);
            await expectPopoverInViewport(page, popoverContent);
        ...
        test("tablet: trigger in center — popover stays in viewport", async ({ mount, page }) => {
            await mount(<div style={{ paddingTop: "450px", paddingLeft: "50px" }}>
          <CitationComponent citation={baseCitation} verification={verificationWithTallImage} pageImagesByAttachmentId={pageImagesByAttachmentId}/>
        </div>);
            const { popoverContent } = await expandToFullPage(page);
            await expectPopoverInViewport(page, popoverContent);
        ...
        test("tablet: trigger near bottom — popover stays in viewport", async ({ mount, page }) => {
            await mount(<div style={{ paddingTop: "900px", paddingLeft: "50px" }}>
          <CitationComponent citation={baseCitation} verification={verificationWithTallImage} pageImagesByAttachmentId={pageImagesByAttachmentId}/>
        </div>);
            const { popoverContent } = await expandToFullPage(page);
            await expectPopoverInViewport(page, popoverContent);
        ...
        test("tablet: image width fills available space", async ({ mount, page }) => {
            await mount(<div style={{ paddingTop: "100px", paddingLeft: "50px" }}>
          <CitationComponent citation={baseCitation} verification={verificationWithTallImage} pageImagesByAttachmentId={pageImagesByAttachmentId}/>
        </div>);
            const { popover } = await expandToFullPage(page);
    ...
    test.describe("Tablet landscape (1024×768)", () => {
        ...
        test("tablet landscape: trigger in center — popover stays in viewport", async ({ mount, page, }) => {
            await mount(<div style={{ paddingTop: "300px", paddingLeft: "100px" }}>
          <CitationComponent citation={baseCitation} verification={verificationWithTallImage} pageImagesByAttachmentId={pageImagesByAttachmentId}/>
        </div>);
            const { popoverContent } = await expandToFullPage(page);
...
test.describe("Expanded-Page Popover Sizing", () => {
    ...
    test("expanded-page stays centered on trigger when viewport has room", async ({ mount, page }) => {
        await mount(<div style={{ paddingTop: "260px", display: "flex", justifyContent: "center" }}>
        <GeneratedImageCitation width={520} height={300}/>
      </div>);
        const { popoverContent } = await expandToFullPage(page);
    ...
    test("expanded width adapts across viewport resize", async ({ mount, page }) => {
        ...
        const { popoverContent } = await expandToFullPage(page);
        const readWidth = async () => popoverContent.evaluate(el => el.getBoundingClientRect().width);
        ...
        const reclampGuard = () => page.evaluate(() => {
