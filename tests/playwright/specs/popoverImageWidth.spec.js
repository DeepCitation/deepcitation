...
const wideImageBase64 = "data:image/png;base64,";
const verificationWithWideImage = {
    status: "found",
...
const verificationWithPartialMatch = {
...
test.describe("Popover Image Keyhole Strip", () => {
    test("popover container has constrained width", async ({ mount, page }) => {
        await mount(<div style={{ padding: "100px" }}>
        ...
        const containerWidth = await container.evaluate(el => parseFloat(window.getComputedStyle(el).width));
    ...
    test("keyhole strip has fixed height", async ({ mount, page }) => {
        ...
        const stripHeight = await strip.evaluate(el => parseFloat(window.getComputedStyle(el).height));
    ...
    test("keyhole strip uses contrasted canvas background in light mode", async ({ mount, page }) => {
        ...
        await expect(strip).toBeVisible();
        const backgroundColor = await strip.evaluate(el => window.getComputedStyle(el).backgroundColor);
    ...
    test("image renders at natural scale (not squashed)", async ({ mount, page }) => {
        ...
        await expect.poll(async () => image.evaluate(el => el.style.width)).not.toBe("");
        // Image should NOT use object-fit (no squashing)
        const objectFit = await image.evaluate(el => el.style.objectFit);
    ...
    test("strip container uses overflow:hidden for drag-only panning", async ({ mount, page }) => {
        ...
        const overflowX = await strip.evaluate(el => window.getComputedStyle(el).overflowX);
        expect(overflowX).toBe("hidden");
        const overflowY = await strip.evaluate(el => window.getComputedStyle(el).overflowY);
...
test.describe("Pre-render boundary alignment", () => {
    ...
    test("summary opens without first-frame position snap", async ({ mount, page }) => {
        await mount(<div style={{ paddingTop: "320px", display: "flex", justifyContent: "center" }}>
        <CitationComponent citation={baseCitation} verification={verificationWithWideImage}/>
      </div>);
        await page.evaluate(() => {
            window.__dcPopoverFrameSamples = [];
            const sample = () => {
                const wrapper = document.querySelector("[data-dc-popover-wrapper]");
                if (wrapper) {
                    const matrix = new DOMMatrixReadOnly(window.getComputedStyle(wrapper).transform);
                    window.__dcPopoverFrameSamples?.push({
                        x: matrix.m41,
                        y: matrix.m42,
                    });
                }
                if ((window.__dcPopoverFrameSamples
                    ?.length ?? 0) < 4) {
        ...
        const samples = await page.evaluate(() => window.__dcPopoverFrameSamples ?? []);
    ...
    test("summary near right edge stays in bounds without guard translation", async ({ mount, page }) => {
        await mount(<div style={{ paddingTop: "320px", display: "flex", justifyContent: "flex-end", paddingRight: "8px" }}>
        <CitationComponent citation={baseCitation} verification={verificationWithWideImage}/>
      </div>);
        await page.locator("[data-citation-id]").click();
        ...
        const guardTranslate = await popover.evaluate(el => el.style.translate || "");
    ...
    test("summary width remains adaptive (not forced full usable width)", async ({ mount, page }) => {
        await mount(<div style={{ padding: "48px" }}>
        <CitationComponent citation={baseCitation} verification={verificationWithWideImage}/>
      </div>);
        await page.locator("[data-citation-id]").click();
        const popover = page.getByRole("dialog");
...
test.describe("Popover Visual States", () => {
    ...
    test("popover with verified image on click", async ({ mount, page }) => {
        test.skip(skipVisualTests, "Skipping visual test - no baseline for this platform");
        await mount(<div style={{ padding: "150px 50px" }}>
        <CitationComponent citation={baseCitation} verification={verificationWithWideImage}/>
      </div>);
        const citation = page.locator("[data-citation-id]");
...
test.describe("Image Click to Expand", () => {
    test("clicking image within popover expands to inline evidence view", async ({ mount, page }) => {
        await mount(<div style={{ padding: "50px" }}>
        <CitationComponent citation={baseCitation} verification={verificationWithWideImage}/>
      </div>);
