...
const verificationWithEvidence = {
    status: "found",
    label: "Q4_Report.pdf",
    attachmentId,
    document: {
...
test.describe("Page Pill Interactions", () => {
    test("summary shows PagePill button when page images exist", async ({ mount, page }) => {
        await mount(<CitationComponent citation={documentCitation} verification={verificationWithEvidence} pageImagesByAttachmentId={{ [attachmentId]: pageImages }}/>);
    ...
    test("page pill is static text when no page images", async ({ mount, page }) => {
        ...
        await mount(<CitationComponent citation={documentCitation} verification={noImageVerification}/>);
        const citation = page.locator("[data-citation-id]");
        await citation.click();
        const popover = page.getByRole("dialog");
        await expect(popover).toBeVisible({ timeout: 10000 });
